...
Class HtmlFixer {
	...
	public function setar_diretorio_imagems($diretorio)
	{
		$this->dir_imagens = $diretorio;
	}
		...
		*/
		...
		if($left=="SRC")
		{
			...
			if($this->dir_imagens!="")
			{
				...
				$imagens = explode("/",$esq[1]);
				$imagens = $imagens[1];

				$imagem = (substr($this->dir_imagens,-1,1)=="/" ? $this->dir_imagens."".$imagens : $this->dir_imagens ."/".$imagens);
				$right = "=".$imagem;
			}
		}
	...
	public function chars_entre($tag1,$tag2,$tutto)
	{
		return $this->extractChars($tag1,$tag2,$tutto);
	}

	private function mergeStyleAttributes($s,$tagType) {
		//
		...
		if($tagType=="img")
		{
			//echo "s-$s-<br/>x=".$x."<br/>-$c-";
			if($c==0)
			{
				$s = substr($s, 0,-2) . " style=\"display:block;\" />";
			}
			else
			{
				...
				*/
				...
				if($s[$i]==">") {
					...
					*/
					...
					$tag = $this->mergeStyleAttributes($tag,$tagType);
