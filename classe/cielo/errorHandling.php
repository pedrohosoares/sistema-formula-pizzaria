<?php
  ...
  {
  	$logFile = "/../../log/cielo/gateway_cielo.log";
  }
  else
  {
  	$logFile = "log/cielo/gateway_cielo.log";
	...
	function VerificaErro($vmPost, $vmResposta)
	{
		...
		catch (Exception $ex)
		{
			...
			echo '</pre><p><center>';
			echo '<input type="button" value="Fechar" onclick="javascript:if(window.opener!=null){window.opener.location.reload();' . 
...
/*
		if($objResposta->getName() == "erro")
		{
			...
			//echo "     XML de envio: " . "<br />" . htmlentities($vmPost);
			echo '</pre><p><center>';
			//echo '<input type="button" value="Retornar" onclick="javascript:if(window.opener!=null){window.opener.location.reload();' . 'window.close();}else{window.location.href=' . "'index.php';" . '}" />';
