...
$cod_cupons = validaVarGet('cc', '/[0-9]+/');

$conexao = conectabd();
$sqlCupom = "SELECT * FROM ipi_cupons  WHERE cod_cupons = '$cod_cupons'";
$resCupom =  mysql_query($sqlCupom);
$linCupom =  mysql_num_rows($resCupom);
$objCupom = mysql_fetch_object($resCupom);

if ((date("Y-m-d", strtotime($objCupom->data_validade)) < date("Y-m-d")) || (date("Y-m-d", strtotime($objCupom->data_inicio))>date("Y-m-d"))):

  echo "Este cupom encontra-se vencido ou ainda não pode ser utilizado!";

else:

//echo '<pre>'.$cod_cupons.' > Cupom:<br>';
//print_r($objCupom);
//echo '</pre>';
...
<?
	...
	{
	?>
	$('sabor<? echo $j; ?>_pizza').addEvent('change', function(e) {
		new Event(e).stop();
...
function carregarCombo(idCombo)
{
  ...
  $('#'+idCombo).empty().append('<option value="">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Carregando...<\/option>');
	...
	var var_url='tipo=carregarCombo&cod='+idCombo+'&tamanho_pizza='+document.getElementById('tam_pizza').value + '&promo=nao';
		...
		var var_url='tipo=carregarCombo&cod='+idCombo+'&tamanho_pizza='+document.getElementById('tam_pizza').value + '&promo=nao&cupom=sim';
  jQuery.ajax({
    type: "POST", 
    url: "ipi_req_carrinho_pedido_ajax.php",
    data: var_url,
...
function carregarPizzas(id)
{
	...
	var var_url='tipo=carregarPizzas&tamanho_pizza='+document.getElementById('tam_pizza').value+'&id_pizza='+id;
  jQuery.ajax({
    type: "POST",
    url: "ipi_req_carrinho_pedido_ajax.php",
    data: var_url,
...
<input type="hidden" name="cc" id="cc" value="<? echo validaVarGet('cc', '/[0-9]+/'); ?>">
<input type="hidden" name="cpp" id="cpp" value="0">
  ...
  <div id="carrinho_fundo_pedido" class="box_pedido">
    ...
    <div id="carrinho_selecao_opcoes" class="box_pizza">
      ...
      <div id="carrinho_tam_pizza">
        ...
        <select name="tam_pizza" id="tam_pizza" onchange="javascript:tamanho_pizza(this);" style="width: 220px;">
          <option value="0">Selecione</option>
				  <?
				  $sqlBordas = "SELECT * FROM ipi_tamanhos t WHERE cod_tamanhos=".$objCupom->cod_tamanhos." ORDER BY t.tamanho";
      ...
      <div id="numero_sabores" style="display: none;">
	      <label>Quantos Sabores?
        <select name="num_sabores" id="num_sabores" onchange="javascript:verificar_num_sabores(this);" style="width: 220px">
          <option value="0">Selecione</option>
        </select>
      </label>
      </div>
    ...
    <!--   <div id="td_gergelim" style="display: none;">
    		<label>Gergelim na Borda?
	      <select name="gergelim" id="gergelim" onchange="javascript:exibir_tipo_massa();" style="width: 155px;">
          <option value="0">Selecione</option>
        </select>
      </div> -->
   ...
   <!--    <div id="td_tipo_massa" style="display: none">
        <label>Tipo da Massa?
        <select name="tipo_massa" id="tipo_massa" onchange="javascript:exibir_cortes();" style="width: 170px;">
          <option value="0">Selecione</option>
        </select>
      </div> -->
    ...
    <table border="0" width="620">
     ...
     <!-- <div id='sabores_indice' style="display: none"> onChange="javascript:carregar_ingredientes(<? echo $j; ?>);" -->
        <div id='titulo_sabor_vazio' class='sabores_titulo' ></div> 
          ...
          <div id="carrinho_ingredientes_texto<? echo $j; ?>" style="padding-bottom:75px" class="carrinho_ingredientes_texto sabores_titulo">
  ...
  <div class='div_botoes_pedido'>
    <div id='botoes_pedido' style='display:none; margin-top: 20px;'>
      <a  href="javascript:void(0)" onClick="javascript:frmCarrinho.submit()" class="btn btn-secondary">Adicionar ao Pedido</a>
<br/><br/>
  </div>
  </div>
  	...
  	<?
	if (validaVarGet('proximo')=="b")
	{
 		echo '<input type="hidden" name="acao" value="adicionar_promocao_bebidas">';
	}
 	else
 	{
 		echo '<input type="hidden" name="acao" value="adicionar_promocao">';
