      ...
      <tbody>
        ...
        if($acao!="")
        {
          ...
          while($objBuscaRegistros = mysql_fetch_object($resBuscaRegistros)) {
            ...
            if(!empty($arquivoJson) and isset($arquivoJson['caminho_danfe']) and isset($arquivoJson['qrcode_url'])){
              echo '<td align="center" '.$cor.'><a target="_blank" href="https://api.focusnfe.com.br'.$arquivoJson['caminho_danfe'].'" style="display:block;border: 1px solid #333;margin: 3px;background: yellow;padding: 2px;">CUPOM FISCAL</a><a target="_blank" href="'.$arquivoJson['qrcode_url'].'" style="display:block;border: 1px solid #333;margin: 3px;background: yellow;padding: 2px;">NOTA FISCAL</a></td>';
              ...
              $data = explode(' ', $data);
              $data = explode('-', $data[0]);
              $pasta = 'http://formulasys.encontresuafranquia.com.br/'.$cnpj.'/'.$data[0].'/'.$data[1].'/'.$data[2].'/'.$objBuscaRegistros->cod_pedidos.'.pdf';
              echo '<td align="center" '.$cor.'><a target="_blank" href="'.$pasta.'" style="display:block;border: 1px solid #333;margin: 3px;background: yellow;padding: 2px;">CUPOM PDF</a></td>';
              ?>
              ...
              <?php
            }else if(isset($arquivoJson['caminho_xml_cancelamento'])){
              ...
              echo '<td align="center" '.$cor.'>FALHA NA EMISSÃO ref:'.$objBuscaRegistros->ref_nota_fiscal.'</td>';
              echo '<td align="center" '.$cor.'>FALHA NA EMISSÃO ref:'.$objBuscaRegistros->ref_nota_fiscal.'</td>';
            }
