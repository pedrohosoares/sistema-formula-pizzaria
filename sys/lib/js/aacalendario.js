...
var vlaDatePicker = new Class({
	...
	position: function() {
		...
		// Este trexo comentado está dando problemas com o nosso tab, por isso fiz outro código
		/*
		switch(this.alignX) {
			case 'left':
	    ...
	    */
		...
		left = this.element.getCoordinates().left + this.element.getSize().x;
	...
	hide: function() {
		if(this.visible) {
			...
			this.picker.set('tween', { onComplete: function() { 
			  ...
			  //Meu hack!
			  _class.fireEvent('hide', null, 10);
			   
			}, duration: this.toggleDuration }).fade('out');
		}
	},
...
vlaDatePicker.implement(new Events);