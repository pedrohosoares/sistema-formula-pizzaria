<?php
  ...
  $decadeAr	= array( array(1939, 1950),
             array(1949, 1960),
             array(1959, 1970),
             array(1969, 1980),
             array(1979, 1990),
...
?>
<table class="year" cellpadding="0" summary="{'ts': '<?=$ts?>', 'pr_ts': '<?=$pr_ts?>', 'nx_ts': '<?=$nx_ts;?>', 'label': '<?=($decade[0]+1).' - '.($decade[1]-1)?>', 'current': 'decade'<?=($decade == $decadeAr[0] ? ", 'hide_left_arrow': '1'" : '').($decade == $decadeAr[sizeof($decadeAr)-1] ? ", 'hide_right_arrow': '1'" : '')?>}">
<?php
	//Add decades
	$year = $decade[0];
	for($i = 0; $i < 3; $i++) {
