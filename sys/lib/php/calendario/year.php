...
?>
<table class="year" cellpadding="0" summary="{'ts': '<?=$ts?>', 'pr_ts': '<?=$pr_ts?>', 'nx_ts': '<?=$nx_ts;?>', 'label': '<?=$ts_year?>', 'current': 'year', 'parent': 'decade'<?=($ts_year == 1979 ? ", 'hide_left_arrow': '1'" : '').($ts_year == 2030 ? ", 'hide_right_arrow': '1'" : '')?>}">
<?php
	//Add years
	$m = 1;
	for($i = 0; $i < 3; $i++) {
