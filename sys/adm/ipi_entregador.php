...
cabecalho('Cadastro de Entregadores');

$acao = validaVarPost('acao');

$tabela = 'ipi_entregadores';
$chave_primaria = 'cod_entregadores';
...
switch ($acao)
{
    ...
    case 'editar':
        ...
        $moto_modelo = validaVarPost('moto_modelo');
        $moto_marca = validaVarPost('moto_marca');
        $moto_placa = validaVarPost('moto_placa');
        $numero_cadastro = validaVarPost('numero_cadastro');
        $conexao = conectabd();
        
        if ($codigo <= 0)
        {
            //$sql_edicao = sprintf("INSERT INTO $tabela (cod_pizzarias, nome, numero_cadastro) VALUES ('%s', '%s', '%s')", $cod_pizzarias, $nome, $numero_cadastro);

            $sql_edicao = sprintf("INSERT INTO $tabela (cod_pizzarias, nome, cpf, cep, endereco, numero , complemento, bairro, cidade, estado, telefone_residencial, telefone_recado, celular, contato_emergencia, telefone_emergencia, moto_modelo, moto_marca, moto_placa, situacao, data_nascimento, sexo, rg, estado_civil, tem_filhos, escolaridade, fumante, bebida_alcoolica, animais_estimacao, numero_cadastro,balcao) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s')", 
            $cod_pizzarias, $nome, $cpf, $cep, $endereco, $numero, $complemento, $bairro, $cidade, $estado, $telefone_residencial, $telefone_recado, $celular, $contato_emergencia, $telefone_emergencia, $moto_modelo, $moto_marca, $moto_placa, $situacao, $data_nascimento, $sexo, $rg, $estado_civil, $tem_filhos, $escolaridade, $fumante, $bebida_alcoolica, $animais_estimacao, $numero_cadastro,$balcao);
        ...
        {
            $sql_edicao = sprintf("UPDATE $tabela SET cod_pizzarias = '%s', nome = '%s', cpf = '%s', cep = '%s', endereco = '%s', numero = '%s', complemento = '%s', bairro = '%s', cidade = '%s', estado = '%s', telefone_residencial = '%s', telefone_recado = '%s', celular = '%s', contato_emergencia = '%s', telefone_emergencia = '%s', moto_modelo = '%s', moto_marca = '%s', moto_placa = '%s', situacao = '%s', data_nascimento = '%s', sexo = '%s', rg = '%s', estado_civil = '%s', tem_filhos = '%s', escolaridade = '%s', fumante = '%s', bebida_alcoolica = '%s', animais_estimacao = '%s', numero_cadastro = '%s',balcao='%s' WHERE $chave_primaria = $codigo", 
            $cod_pizzarias, $nome, $cpf, $cep, $endereco, $numero, $complemento, $bairro, $cidade, $estado, $telefone_residencial, $telefone_recado, $celular, $contato_emergencia, $telefone_emergencia, $moto_modelo, $moto_marca, $moto_placa, $situacao, $data_nascimento, $sexo, $rg, $estado_civil, $tem_filhos, $escolaridade, $fumante, $bebida_alcoolica, $animais_estimacao, $numero_cadastro,$balcao);
...
window.addEvent('domready', function()
{
    ...
    tabs.addEvent('change', function(indice)
    {
        if(indice == 1)
        {
            ...
            document.frmIncluir.numero_cadastro.value = '';
            ...
            document.frmIncluir.moto_modelo.value = '';
            document.frmIncluir.moto_marca.value = '';
            document.frmIncluir.moto_placa.value = '';
...
<? endif; ?>
        ...
        $sql_buscar_registros = "SELECT * FROM $tabela WHERE $opcoes LIKE '%$filtro%' AND cod_pizzarias IN(".implode(',',$_SESSION['usuario']['cod_pizzarias']).")";
...
<table align="center" class="caixa" cellpadding="0" cellspacing="0">
    ...
    <tr>
		<td class="legenda tdbl tdbt tdbr" colspan="2"><label class="requerido"
			for="cod_pizzarias"><? echo ucfirst(TIPO_EMPRESAS)?></label></td>
	</tr>
	<tr>
    ...
    <tr>
        <td class="tdbl tdbr sep">
            <input type="text" name="cpf" id="cpf" maxlength="18" size="45" value="<?echo bd2texto($obj_editar->cpf)?>">
        </td>
    </tr>
    ...
    <select name="escolaridade" id="escolaridade">
        ...
        <option value="Ensino médio incompleto" <? if($obj_editar->escolaridade == 'Ensino médio incompleto') echo 'SELECTED' ?>>Ensino médio incompleto</option>
    ...
    <tr><td class="legenda tdbl tdbr"><label for="balcao">É BALCÃO?</label></td></tr>
    <tr><td class="tdbl tdbr">
    <select name="balcao" id="balcao">
        <option value="NAO"<? if($obj_editar->balcao == 'NAO') echo 'SELECTED' ?>>NÃO</option>
        <option value="SIM"<? if($obj_editar->balcao == 'SIM') echo 'SELECTED' ?>>SIM</option>
    </select>
    </td></tr>
    <tr>
        <td class="legenda tdbl tdbr">
   ...
   <tr>
    ...
    <tr>
        <td class="tdbl tdbr">
            <input type="text" name="numero_cadastro" id="numero_cadastro" maxlength="10" size="15" value="<?echo bd2texto($obj_editar->numero_cadastro)?>">
       </td>
    </tr>
   
    <tr>
        <td class="legenda tdbl tdbr">
            <label for="moto_modelo">Modelo da Moto</label>
        </td>
    </tr>
    <tr>
        <td class="tdbl tdbr">
            <input type="text" name="moto_modelo" id="moto_modelo" maxlength="10" size="15" value="<?echo bd2texto($obj_editar->moto_modelo)?>">
       </td>
    </tr>
   
   <tr>
        <td class="legenda tdbl tdbr">
            <label for="moto_marca">Marca da Moto</label>
        </td>
    </tr>
    <tr>
        <td class="tdbl tdbr">
            <input type="text" name="moto_marca" id="moto_marca" maxlength="10" size="15" value="<?echo bd2texto($obj_editar->moto_marca)?>">
       </td>
    </tr>
    
    <tr>
        <td class="legenda tdbl tdbr">
            <label for="moto_placa">Placa da Moto</label>
        </td>
    </tr>
    <tr>
        <td class="tdbl tdbr">
            <input type="text" name="moto_placa" id="moto_placa" maxlength="8" size="15" value="<?echo bd2texto($obj_editar->moto_placa)?>">
       </td>
    </tr>
