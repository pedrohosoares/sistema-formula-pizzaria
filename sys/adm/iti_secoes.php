...
cabecalho('Cadastro Secões');

$acao = validaVarPost('acao');

$chave_primaria = 'cod_secoes';
$tabela = 'iti_secoes';
$campo_ordenacao = 'secao';
$campo_filtro_padrao = 'secao';
$quant_pagina = 50;
...
switch ($acao)
{
    ...
    case 'editar':
        ...
        $secao = validaVarPost('secao');
        $exibir_data_hora = validaVarPost('exibir_data_hora') ? 1 : 0;
        
        $conexao = conectabd();
        
        if ($codigo <= 0)
        {
            $sql_edicao = sprintf("INSERT INTO $tabela (secao, exibir_data_hora) VALUES ('%s', '%s')", $secao, $exibir_data_hora);
        ...
        {
            $sql_edicao = sprintf("UPDATE $tabela SET secao = '%s', exibir_data_hora = '%s' WHERE $chave_primaria = $codigo", $secao, $exibir_data_hora);
            if (mysql_query($sql_edicao))
            {
                mensagemok('Registro adicionado com êxito!');
            }
...
window.addEvent('domready', function(){
  ...
  tabs.addEvent('change', function(indice){
    ...
    echo $chave_primaria?>.value = '';
      document.frmIncluir.secao.value = '';
      document.frmIncluir.exibir_data_hora.value = '';
        ...
        <table class="listaEdicao" cellpadding="0" cellspacing="0"
            ...
            <thead>
                <tr>
                    ...
                    <td align="center" width="130">Exibir Data e Hora</td>
                </tr>
            </thead>
            <tbody>
            ...
            while ($obj_buscar_registros = mysql_fetch_object($res_buscar_registros))
            {
                ...
                echo '<td align="center"><a href="#" onclick="editar(' . $obj_buscar_registros->$chave_primaria . ')">' . bd2texto($obj_buscar_registros->secao) . '</a></td>';
                
                if ($obj_buscar_registros->exibir_data_hora == 1)
                {
...
<table align="center" class="caixa" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td class="legenda tdbl tdbt tdbr"><label class="requerido" for="secao">Seção</label></td>
    </tr>
    <tr>
        <td class="tdbl tdbr sep"><input class="requerido" type="text" name="secao" id="secao" maxlength="45" size="45" value="<? echo bd2texto($obj_editar->secao)?>"></td>
    </tr>

    <tr>
        <td class="legenda tdbl tdbr" colspan="2"><label
            for="exibir_data_hora">Exibir Data e Hora</label></td>
    </tr>
    <tr>
        <td class="tdbl tdbr" colspan="2">
            <select class="input" name="exibir_data_hora">
                <option value="0" <? if($obj_editar->exibir_data_hora == '0') echo 'SELECTED' ?>>Não</option>  
                <option value="1" <? if($obj_editar->exibir_data_hora == '1') echo 'SELECTED' ?>>Sim</option>
            </select>
        </td>
    </tr>
