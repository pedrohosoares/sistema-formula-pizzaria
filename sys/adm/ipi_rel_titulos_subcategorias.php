...
cabecalho('Relatório de Títulos x SubCategorias');
...
<table class="listaEdicao" cellpadding="0" cellspacing="0" style="width: 600px; margin: 0px auto;" align="center">
    ...
    while ($obj_categorias = mysql_fetch_object($res_categorias))
    {
        ...
        $sql_total_cat = "SELECT SUM(tp.valor) total FROM ipi_titulos_parcelas tp INNER JOIN ipi_titulos t ON (tp.cod_titulos = t.cod_titulos) WHERE t.tipo_titulo='PAGAR' AND t.cod_titulos_subcategorias IN ( SELECT cod_titulos_subcategorias FROM ipi_titulos_subcategorias WHERE t.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND cod_titulos_categorias = ".$obj_categorias->cod_titulos_categorias.")";
        //echo $sql_total_cat."<br/><br/>";
      
        if ($tipo_data_filtro == 'VENC')
        {
            $sql_total_cat .= " AND tp.data_vencimento BETWEEN '" . data2bd($data_inicial_filtro) . "' AND '" . data2bd($data_final_filtro) . "'";
        }
        else if($tipo_data_filtro == 'REF')
        {
            $sql_total_cat .= " AND tp.mes_ref BETWEEN MONTH('" . data2bd($data_inicial_filtro) . "') AND MONTH('" . data2bd($data_final_filtro) . "') AND tp.ano_ref BETWEEN YEAR('" . data2bd($data_inicial_filtro) . "') AND YEAR('" . data2bd($data_final_filtro) . "')";
        }
        ...
        echo '<td align="right"><b><i>' . bd2moeda(abs($obj_total_cat->total)) . '</i></b></td>';
        echo '</tr>';
        ...
        {
            $sql_total_sub = "SELECT SUM(tp.valor) total FROM ipi_titulos_parcelas tp INNER JOIN ipi_titulos t ON (tp.cod_titulos = t.cod_titulos) WHERE t.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND t.cod_titulos_subcategorias = ".$obj_subcat->cod_titulos_subcategorias."";
            
            if ($tipo_data_filtro == 'VENC')
            {
                $sql_total_sub .= " AND tp.data_vencimento BETWEEN '" . data2bd($data_inicial_filtro) . "' AND '" . data2bd($data_final_filtro) . "'";
            }
            else if($tipo_data_filtro == 'REF')
            {
                $sql_total_sub .= " AND tp.mes_ref BETWEEN MONTH('" . data2bd($data_inicial_filtro) . "') AND MONTH('" . data2bd($data_final_filtro) . "') AND tp.ano_ref BETWEEN YEAR('" . data2bd($data_inicial_filtro) . "') AND YEAR('" . data2bd($data_final_filtro) . "')";
            }
            ...
            {
                $sql_total_sub .= " AND (t.numero_nota_fiscal = '' OR t.numero_nota_fiscal IS NULL)";
            }
            ...
            $total += abs($obj_total_sub->total);
            
            if ($detalhado)
            {
                $sql_titulos = "SELECT tp.*, t.* FROM ipi_titulos_parcelas tp INNER JOIN ipi_titulos t ON (tp.cod_titulos = t.cod_titulos) WHERE t.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND t.cod_titulos_subcategorias = ".$obj_subcat->cod_titulos_subcategorias;
                
                if ($tipo_data_filtro == 'VENC')
                {
                    $sql_titulos .= " AND tp.data_vencimento BETWEEN '" . data2bd($data_inicial_filtro) . "' AND '" . data2bd($data_final_filtro) . "'";
                }
                else if($tipo_data_filtro == 'REF')
                {
                    $sql_titulos .= " AND tp.mes_ref BETWEEN MONTH('" . data2bd($data_inicial_filtro) . "') AND MONTH('" . data2bd($data_final_filtro) . "') AND tp.ano_ref BETWEEN YEAR('" . data2bd($data_inicial_filtro) . "') AND YEAR('" . data2bd($data_final_filtro) . "')";
                }
                ...
                {                    
                    /*if($obj_subcat->titulos_subcategorias == "REQUEIJÃO")
                    {
                    echo $sql_titulos;
                    die();
                    }*/
                    ...
                    {
                    	$obj_buscar_colaborador = executaBuscaSimples("SELECT * FROM ipi_colaboradores WHERE cod_colaboradores = '" . $obj_titulos->cod_colaboradores . "'", $conexao);
        ...
        echo '<td style="padding-left: 80px;" align="right"><b>' . bd2moeda($total) . '</b></td>';
        echo '</tr>';
    }
