...
$tabela = 'ipi_comunicacao_situacoes';
...
switch ($acao)
{
    ...
    case 'editar':
        ...
        if ($codigo <= 0)
        {
            $sql_edicao = sprintf("INSERT INTO $tabela (nome_situacao,situacao) VALUES ('%s','%s')", $nome_situacao,$status);
        ...
        {
            $sql_edicao = sprintf("UPDATE $tabela SET nome_situacao = '%s', situacao = '%s' WHERE $chave_primaria = $codigo", $nome_situacao, $status);
            $res_edicao = mysql_query($sql_edicao);
        }
...
<? endif; ?>
        ...
        $sql_buscar_registros .= ' ORDER BY nome_situacao LIMIT ' . ($quant_pagina * $pagina) . ', ' . $quant_pagina;
        $res_buscar_registros = mysql_query($sql_buscar_registros);
        ...
        <table class="listaEdicao" cellpadding="0" cellspacing="0"
            ...
            <thead>
                <tr>
                    ...
                    <td align="center" width="30">Nome da Situação</td>
            ...
            while ($obj_buscar_registros = mysql_fetch_object($res_buscar_registros))
            {
                ...
                echo '<td align="center"><a href="#" onclick="editar(' . $obj_buscar_registros->$chave_primaria . ')">' . bd2texto($obj_buscar_registros->nome_situacao) . '</a></td>';
...
<table align="center" class="caixa" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td class="legenda tdbl tdbt tdbr"><label class="requerido" for="nome_situacao">Nome da Categoria</label></td>
    </tr>
    <tr>
        <td class="tdbl tdbr"><input class="requerido" type="text" name="nome_situacao" id="nome_situacao" maxlength="100" size="45" value="<? echo bd2texto($obj_editar->nome_situacao)?>"></td>
    </tr>
    
    <tr>
        <td class="legenda tdbr tdbl"><label for="status">Situação</label></td>
    </tr>
    <tr>
