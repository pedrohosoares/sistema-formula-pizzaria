...
window.addEvent('domready', function() 
{
  ...
  formas_pagamento.setDataURL('ipi_rel_forma_pagamentos_ajax_v2.php?param=1,'+data_inicial+','+data_final+','+cod_pizzarias);
  ...
  net_tel.setDataURL('ipi_rel_forma_pagamentos_ajax_v2.php?param=2,'+data_inicial+','+data_final+','+cod_pizzarias);
 ...
 <table align="center" class="caixa" cellpadding="0" cellspacing="0">
  ...
  <table class="listaEdicao" cellpadding="0" cellspacing="0" width="600">
    ...
    */
    ...
    $objBuscaPedidosSoma = executaBuscaSimples("SELECT SUM(pfp.valor) AS soma_tel FROM ipi_pedidos p LEFT JOIN ipi_pedidos_formas_pg pfp ON (p.cod_pedidos = pfp.cod_pedidos) WHERE p.data_hora_pedido BETWEEN '$data_inicial 00:00:00' AND '$data_final 23:59:59' AND p.origem_pedido = 'TEL' AND p.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND p.situacao = 'BAIXADO' $SqlCodPizzarias", $con);
    $valor_total_tel = $objBuscaPedidosSoma->soma_tel;
    
    $objBuscaPedidosSoma = executaBuscaSimples("SELECT SUM(pfp.valor) AS soma_net FROM ipi_pedidos p LEFT JOIN ipi_pedidos_formas_pg pfp ON (p.cod_pedidos = pfp.cod_pedidos) WHERE p.data_hora_pedido BETWEEN '$data_inicial 00:00:00' AND '$data_final 23:59:59' AND p.origem_pedido IN ('NET','IFOOD') AND p.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND p.situacao = 'BAIXADO' $SqlCodPizzarias", $con);
    $valor_total_net = $objBuscaPedidosSoma->soma_net;
    $valor_total_geral =  $valor_total_tel + $valor_total_net;
    // Totais pra efeito de calculo de porcentagens
    ...
    for ($a = 0; $a < $num_formas_pg; $a++)
    {
        ...
        <tr>
            ...
            <?
            $objBuscaPedidosSoma = executaBuscaSimples("SELECT SUM(pfp.valor) AS soma_tel FROM $tabela p LEFT JOIN ipi_pedidos_formas_pg pfp ON (p.cod_pedidos = pfp.cod_pedidos) WHERE p.data_hora_pedido BETWEEN '$data_inicial 00:00:00' AND '$data_final 23:59:59' AND pfp.cod_formas_pg = '".$obj_formas_pg->cod_formas_pg."' AND p.origem_pedido = 'TEL' AND p.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND p.situacao = 'BAIXADO' $SqlCodPizzarias", $con);
            $soma_tel = $objBuscaPedidosSoma->soma_tel;
            $total_tel += $soma_tel;
            
            $objBuscaPedidosSoma = executaBuscaSimples("SELECT SUM(pfp.valor) AS soma_net FROM $tabela p LEFT JOIN ipi_pedidos_formas_pg pfp ON (p.cod_pedidos = pfp.cod_pedidos) WHERE p.data_hora_pedido BETWEEN '$data_inicial 00:00:00' AND '$data_final 23:59:59' AND pfp.cod_formas_pg = '".$obj_formas_pg->cod_formas_pg."' AND p.origem_pedido IN ('IFOOD','NET') AND p.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND p.situacao = 'BAIXADO' $SqlCodPizzarias", $con);
            $soma_net = $objBuscaPedidosSoma->soma_net;
            $total_net += $soma_net;
            $total_forma_pgto = $soma_tel + $soma_net;
