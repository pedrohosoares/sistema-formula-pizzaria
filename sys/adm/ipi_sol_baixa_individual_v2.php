...
$ifood_polling = validaVarPost('ifood_polling');
$tabela = 'ipi_pedidos';
...
switch($acao) 
{
  ...
  if(!empty($ifood_polling)){
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, CAMINHO_DELIVERY_IFOOD.$ifood_polling);
  ...
  $valor_formas = validaVarPost('txt_valor_formas');
  ...
  $objBuscaPedido = executaBuscaSimples("SELECT *, (SELECT c.cupom FROM ipi_cupons c INNER JOIN ipi_pedidos_ipi_cupons pp ON (pp.cod_cupons = c.cod_cupons)  WHERE pp.cod_pedidos = p.cod_pedidos) cupom FROM ipi_pedidos p WHERE p.cod_pedidos = $cod_pedidos", $con);
   ...
   {*/
    ...
    if($objBuscaPedido->situacao!='BAIXADO')
    {
      ...
      $sql_buscar_forma_pg = "SELECT * FROM ipi_formas_pg ifp INNER JOIN ipi_formas_pg_pizzarias ifpp ON (ifp.cod_formas_pg = ifpp.cod_formas_pg) WHERE ifp.cod_formas_pg = '".$forma_pg[0]."' AND ifpp.cod_pizzarias = ".$objBuscaPedido->cod_pizzarias;
      ...
      */
      $ifood = "SELECT pedido_ifood_json FROM ipi_pedidos WHERE cod_pedidos='".$cod_pedidos."'";
      $ifood = mysql_query($ifood);
      $ifood = mysql_fetch_object($ifood);

      if((($obj_buscar_forma_pg->prazo=="")||($obj_buscar_forma_pg->cod_titulos_subcategorias_taxa<=0 && $obj_buscar_forma_pg->taxa>0)||($obj_buscar_forma_pg->cod_titulos_subcategorias<=0)||($obj_buscar_forma_pg->cod_bancos<=0)) and empty($ifood->pedido_ifood_json))
      {
      ...
      /*
      else if(!empty($ifood->pedido_ifood_json)){
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://formulasys.encontresuafranquia.com.br/ifood.php?acao=delivery&chave=165117047d56ce2487aa718bd8d6c5b7&cod_pedido=".$ifood->ifood_polling);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 0);
        curl_setopt($ch, CURLOPT_POST, 1);
        $body = curl_exec($ch);
        $http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
      ...
      */
        ...
        if(!empty($sqlQueryPegaPedido->ifood_polling)){
          $ch = curl_init();
          curl_setopt($ch, CURLOPT_URL, "https://formulasys.encontresuafranquia.com.br/ifood.php?acao=delivery&chave=165117047d56ce2487aa718bd8d6c5b7&cod_pedido=".$sqlQueryPegaPedido->ifood_polling);
          curl_setopt($ch, CURLOPT_RETURNTRANSFER, 0);
          curl_setopt($ch, CURLOPT_POST, 1);
          $body = curl_exec($ch);
          $http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        ...
        */
        ...
        if (isset($forma_pg))
        {
          ...
          $sql_del_forma_pgto = "DELETE FROM ipi_pedidos_formas_pg WHERE cod_pedidos = '".$cod_pedidos."'";
              //echo "<br>del: ".$sql_del_forma_pgto;
          $res_del_forma_pgto = mysql_query($sql_del_forma_pgto);
          ...
          if ( ($objBuscaPedido->origem_pedido == 'IFOOD') || ($objBuscaPedido->origem_pedido == 'NET') || (PONTOS_FIDELIDADE == "TODOS_PEDIDOS") )
          {
            ...
            if(ENQUETE_ENVIAR == 1)
            {
              ...
              if ($obj_buscar_clientes->email!="")
              {
                ...
                $texto = "<br><br>Obrigado ".bd2texto($obj_buscar_clientes->nome)." pela compra, agora só falta você responder nossa enquete e ganhar uma  ".$nome_produto_visual."* na próxima compra!";
            ...
            if ( ($num_email_fidelidade>0)&&($obj_buscar_clientes->email!="") )
            {
              ...
              $con = conectar_bd();
              $sql_pedidos_formas_pg = "SELECT * FROM ipi_pedidos_formas_pg WHERE cod_pedidos = '".$cod_pedidos."'";
            //echo "<Br>x: ".$sql_pedidos_formas_pg;
              $res_pedidos_formas_pg = mysql_query($sql_pedidos_formas_pg);
              $num_pedidos_formas_pg = mysql_num_rows($res_pedidos_formas_pg);
              $cod_titulos_recebimento = '';
              for ($t=1; $t<=$num_pedidos_formas_pg; $t++)
              {
                $obj_pedidos_formas_pg = mysql_fetch_object($res_pedidos_formas_pg);


              //Inserindo no fluxo de caixa.
              // Lançando considerando a forma de pagamento
                $sql_buscar_forma_pg = "SELECT ifpp.cod_bancos, ifpp.cod_titulos_subcategorias, ifpp.cod_titulos_subcategorias_taxa, ifpp.taxa, ifpp.prazo, ifp.cod_formas_pg, ifp.forma_pg  FROM ipi_formas_pg ifp INNER JOIN ipi_formas_pg_pizzarias ifpp ON (ifp.cod_formas_pg = ifpp.cod_formas_pg) WHERE ifp.cod_formas_pg = '".$obj_pedidos_formas_pg->cod_formas_pg."' AND ifpp.cod_pizzarias = ".$objBuscaPedido->cod_pizzarias;
              //echo "<Br>sql_buscar_forma_pg:      ".$sql_buscar_forma_pg;
                ...
                $forma_pg = $cod_formas_pg->forma_pg;
              ...
              // Insere os créditos
                ...
                //echo "<Br>sql_inserir_titulos:      ".$sql_inserir_titulos;
                  ...
                  $cod_titulos_recebimento = $cod_titulos;
                }
                else
                {
                  $cod_titulos = $cod_titulos_recebimento;
                }
      ...
      <table align="center" border="0" width="500" style="margin: 0 auto">
        ...
        if ($acao == "buscar")
        {
          ...
          $sql_buscar = "SELECT p.*, c.nome,pi.cnpj, pi.nome nome_pizzaria,pi.cod_pizzarias FROM ipi_pedidos p INNER JOIN ipi_clientes c ON (p.cod_clientes=c.cod_clientes) INNER JOIN ipi_pizzarias pi ON (p.cod_pizzarias=pi.cod_pizzarias) WHERE p.cod_pedidos = '$int_num_pedido' AND p.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ")";
    ...
    //echo "<br>1: ".$sql_buscar;
          ...
          if ($num_buscar > 0)
          {
            if ( ( ( ( $obj_buscar->situacao=="ENVIADO") || ( ( $obj_buscar->situacao=="IMPRESSO")  && $obj_buscar->tipo_entrega == "Balcão") )&& ($obj_buscar->forma_pg!="VISANET") && ($obj_buscar->forma_pg!="MASTERCARDNET") && ($obj_buscar->forma_pg!="VISANET-CIELO") && ($obj_buscar->forma_pg!="MASTERCARDNET-CIELO"))  || ($obj_buscar->situacao=="CAPTURADO") )
            {
              if (in_array($obj_buscar->cod_pizzarias,$_SESSION['usuario']['cod_pizzarias']))
              {
                ...
                <table align="center" border="0" style="margin: 0 auto">
                  ...
                  <tr>
                    <td align="right"><b>Valor do Pedido:</b></td>
                    <td>
                      R$ <? echo bd2moeda($obj_buscar->valor); ?>
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td>
                      R$ <? echo bd2moeda($obj_buscar->desconto); ?>
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td>
                      R$ <? echo bd2moeda($obj_buscar->valor_entrega); ?>
                    </td>
                  </tr>
                ...
                <script>
                  function imprimirNotaFiscal(caminho_danfe,pasta,num_pedido){
                    ...
                    console.log(caminho_danfe,pasta,num_pedido);            
                    let url = "<?php echo CAMINHO_INDEX_IFOOD; ?>"+params;
                    ...
                    enviaPedido.onreadystatechange = function() {
                      ...
                      if (this.readyState == XMLHttpRequest.DONE || this.readyState == 4) {
                        document.getElementById('frm_baixar').submit();
                      }else{
                        document.getElementById('frm_baixar').submit();
                      }
                    }
                  ...
                  function chamadaGeracaoImpressaoNotaFiscal(caminho_danfe,pasta,num_pedido,qr_code,status){
                    ...
                    console.log(caminho_danfe,pasta,num_pedido,qr_code,status);
                    let url = "<?php echo CAMINHO_INDEX_IFOOD; ?>"+params;
                    var enviaPedido = new XMLHttpRequest();
                    enviaPedido.open('GET',url,true);
                    enviaPedido.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
                  ...
                  function enviaPedidoNota(cod_pedidos){
                    ...
                    enviaPedido.onreadystatechange = function() {
                      if (this.readyState == XMLHttpRequest.DONE || this.readyState == 4) {
                        if(this.responseText !== '' && this.responseText !== 'existe'){
                          let nota = JSON.parse(this.responseText);
                          console.log(nota);
                          if(nota['status'] == 'autorizado' && nota['status'] != undefined  && nota['status'] != null ){
                            chamadaGeracaoImpressaoNotaFiscal(nota['caminho_danfe'],nota['pasta'],nota['num_pedido'],nota['qr_code'],nota['status']);
                          }else{
                            document.getElementById('frm_baixar').submit();
                          }
                        }else{
                ...
                <script>
                  ...
                  { 
                    ...
                    let enviaPedido = verificaErro();
                    if(enviaPedido === true){
                      if(document.querySelector('input.gerar_nota') != null && document.querySelector('input.gerar_nota').checked == true){
                        document.getElementById('bt_baixar').setAttribute('disabled',true);
                        document.getElementById('bt_baixar').value = "Baixando..";
                        document.getElementById('preloader_nota').style = "display:block;";
                        let cod_pedidos = document.getElementById('num_pedido').value;
                        enviaPedidoNota(cod_pedidos);
                      }else{
                        document.getElementById('frm_baixar').submit();
                      }
                    }
                  ...
                  function adicionar_forma_pagamento()
                  {
                    ...
                    var html_txt = "";
                    html_txt += '<div>';
                    html_txt += '<input type="text" name="txt_valor_formas[]" class="proximo" style="width: 60px;" onkeypress="return formatar_moeda(this, \'.\', \',\', event);" onblur="calcular_total_das_formas_pgto()">';
                    html_txt += '<select name="forma_pg[]" style="width: 200px; margin-left: 4px;" onblur="">';
                    html_txt += '<option value=""></option>';

                    <?php
                    $con = conectabd();
                    $sql_formas_pg = "SELECT fp.forma_pg,fp.cod_formas_pg FROM ipi_formas_pg fp INNER JOIN ipi_formas_pg_pizzarias fpp on fpp.cod_formas_pg = fp.cod_formas_pg WHERE fpp.cod_pizzarias = '".$obj_buscar->cod_pizzarias."' ORDER BY forma_pg";
                ...
                <form name="frm_baixar" id="frm_baixar" method="post" onsubmit="return validar_baixa(this);">
                  ...
                  <table align="center" border="0" width="500" style="margin: 0 auto">
                    ...
                    <tr>
                      ...
                      <td> 
                        <div id="area_formas_pagamento">
                          <?php
                          $con = conectabd();
                          $sql_pedidos_formas_pg = "SELECT * FROM ipi_pedidos_formas_pg WHERE cod_pedidos = '".$obj_buscar->cod_pedidos."'";
                          $res_pedidos_formas_pg = mysql_query($sql_pedidos_formas_pg);
                          $num_pedidos_formas_pg = mysql_num_rows($res_pedidos_formas_pg);
                          if($num_pedidos_formas_pg < 1){
                            $num_pedidos_formas_pg = 1;
                          }
                          for ($t=1; $t<=$num_pedidos_formas_pg; $t++)
                          {
                            $obj_pedidos_formas_pg = mysql_fetch_object($res_pedidos_formas_pg);
                            ...
                            <div id="div_formas_pagtos_<?php echo $t; ?>">
                              <input type="text" name="txt_valor_formas[]" class="proximo" style="width: 60px;" onkeypress="return formatar_moeda(this, '.', ',', event);" value="<?php echo bd2moeda($obj_pedidos_formas_pg->valor); ?>" onblur="calcular_total_das_formas_pgto()">
                              <select name="forma_pg[]" style="width: 200px; margin-left: 1x;">
                                ...
                                while($obj_formas_pg = mysql_fetch_object($res_formas_pg)) 
                                {
                                  ...
                                  if($obj_pedidos_formas_pg->cod_formas_pg == $obj_formas_pg->cod_formas_pg)
                                    echo 'selected';
                              ...
                              if ($t>1)
                              {
                                echo '<input type="button" value="-" onclick="javascript:remover_forma_pagamento(\'div_formas_pagtos_'.$t.'\')" style="width:20px; margin-left: 2px;">';
                              }
                              ?>
                            </div>
                        ...
                        <input type="hidden" name="contador_divs" id="contador_divs" value="<?php echo $t; ?>" />
                      </td>
                    </tr>
                    <tr>
                      <td align="right"> <b>Total das Formas:</b> </td>
                      <td> 
                        <input type="text" name="txt_total_formas" id="txt_total_formas" class="proximo" style="width: 60px;" readonly="readonly" > 
                        <script>calcular_total_das_formas_pgto();</script>
                      </td>
                    </tr>
                    ...
                    <tr>
                      <td colspan="2" style="text-align: center;">&nbsp;
                        ...
                        if($exibe){
                          if(count(explode('autorizado',$obj_buscar->arquivo_json))<2){
                           ?>
                           <input type="checkbox" checked="true" name="gerar_nota" class="gerar_nota" style="height: 31px;float: left;width: 31px;"> 
                           <div style="font-size: 17px;float: left;padding-left: 10px;margin-top: 8px;    font-weight: 600;">Gerar nota fiscal?</div>
                           <?php 
                         }
                       } 
                       ?> 
                   ...
                   <tr>
                    <td align="center" colspan="2">
                      ...
                      <input type="hidden" name="ifood_polling" value="<? echo $obj_buscar->ifood_polling; ?>">
                    </td>
                  </tr>
                </table>
              </form>
            ...
            else
            {
              ...
              <table align="center" border="0" width="500" style="margin: 0 auto">
                ...
                <? if(isset($obj_buscar->ifood_polling)){ ?>
                  <tr>
                    <td align="right"><b>Pedido Ifood:</b></td>
                    <td>
                      Sim
                    </td>
                  </tr>
      ...
      <style type="text/css">
        #preloader_nota{
          position: fixed; display: none; top: 0; left: 0; background: #FFF; width: 100%; height: 100%; text-align: center; padding-top: 18%; font-size: 26px; opacity: 0.9;
        }
      </style>
      <div id="preloader_nota">Baixando..Aguarde</div>
      <? rodape(); ?>
