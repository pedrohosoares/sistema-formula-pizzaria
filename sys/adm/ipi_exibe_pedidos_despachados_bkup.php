         ...
         <table class="listaEdicao" cellpadding="0" cellspacing="0" width="100%">
          ...
          //bc483fa8600278fc82bdbc5cee62c7ca
            ...
            if(isset($_GET['cod_pedidos']) and !empty($_GET['cod_pedidos'])){
              ...
              $conexao = conectabd();
              $query = 'SELECT pe.cod_pedidos,fp.forma_pg FROM ipi_pedidos pe 
              ...
              WHERE  (pe.ref_nota_fiscal IS NULL OR pe.ref_nota_fiscal = "") AND pe.cod_pizzarias IN ('.$pizzarias.') AND pe.cod_pedidos IN ('.$valores.')';
              $db = mysql_query($query);
              while($dados = mysql_fetch_assoc($db)){
                $a[] = $dados;
                $codPedidos1[] = $dados['cod_pedidos'];
              }
              
              /*
              $queryS = 'SELECT pe.cod_pedidos FROM ipi_pedidos pe 
              WHERE  (pe.ref_nota_fiscal IS NULL OR pe.ref_nota_fiscal = "") AND pe.pedido_ifood_json != "" AND pe.cod_pizzarias IN ('.$pizzarias.') AND pe.cod_pedidos IN ('.$valores.')';
              $dbS = mysql_query($queryS);
              while($dados2 = mysql_fetch_assoc($dbS)){
                $b[] = $dados2;
                $codPedidos2[] = $dados2['cod_pedidos'];
              }
              */
...
<script type="text/javascript">
    ...
    let codPedidosIfood = "<?php echo $get; ?>";
    var url = "https://formulasys.encontresuafranquia.com.br/despacharIfood.php?acao=despachar&chave=165117047d56ce2487aa718bd8d6c5b7&cod_pedido="+codPedidosIfood;
    let despachoIfood = new XMLHttpRequest();
