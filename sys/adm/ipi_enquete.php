...
/**
 * ipi_enquete.php: Cadastro de Enquetes
 * 
 * Índice: cod_enquetes
 * Tabela: ipi_enquetes
 */
...
cabecalho('Cadastro de Enquetes');
...
switch($acao) {
  ...
  case 'editar':
    ...
    $enquete = validaVarPost('enquete');
    $situacao = (validaVarPost('situacao') == 'on') ? 'ATIVO' : 'INATIVO';
    
    $con = conectabd();
    
    if($codigo <= 0) {
      $SqlEdicao = sprintf("INSERT INTO $tabela (enquete, situacao) VALUES ('%s', '%s')", 
                           $enquete, $situacao);
      ...
      $SqlEdicao = sprintf("UPDATE $tabela SET enquete = '%s', situacao = '%s' WHERE $chave_primaria = $codigo", 
                            $enquete, $situacao);
...
window.addEvent('domready', function(){
  ...
  tabs.addEvent('change', function(indice){
    if(indice == 1) {
      ...
      document.frmIncluir.enquete.value = '';
      document.frmIncluir.situacao.checked = true;
...
<div id="tabs">
    ...
    <td class="conteudo">
      ...
      <form name="frmExcluir" method="post" onsubmit="return verificaCheckbox(this)">
        ...
        <table class="listaEdicao" cellpadding="0" cellspacing="0">
          ...
          $SqlBuscaRegistros = "SELECT * FROM $tabela ORDER BY $chave_primaria DESC";
          ...
          while ($objBuscaRegistros = mysql_fetch_object($resBuscaRegistros)) {
            ...
            echo '<td align="center"><a href="javascript:;" onclick="editar('.$objBuscaRegistros->$chave_primaria.')">'.bd2texto($objBuscaRegistros->enquete).'</a></td>';
  ...
  <div class="painelTab">
    ...
    <tr><td class="legenda tdbl tdbt tdbr"><label class="requerido" for="enquete">Enquete</label></td></tr>
    <tr><td class="tdbl tdbr sep"><input class="requerido" type="text" name="enquete" id="enquete" maxlength="45" size="47" value="<? echo texto2bd($objBusca->enquete) ?>"></td></tr>
    
    <tr><td class="tdbl tdbr sep"><input type="checkbox" name="situacao" id="situacao" <? if($objBusca->situacao == 'ATIVO') echo 'checked' ?>>&nbsp;<label for="situacao">Ativo</label></td></tr>
