 ...
 * 1.1		 15/04/2010   ELIAS			Melhoria no Script (Imagem e Seção)
 * 1.2		 30/04/2010   ELIAS			Usando apenas o campo imagem_gde, pois deixamos de redimensionar e usar o campo imagem_pna para aceitar mais formatos de imagem.
 */
...
switch ($acao)
{
    case 'excluir':
        ...
        $sql_del = "SELECT * FROM $tabela WHERE $chave_primaria IN ($indices_sql)";
        $res_del = mysql_query($sql_del);
        
        while ($obj_del = mysql_fetch_object($res_del)) 
        {
            if(is_file(UPLOAD_DIR.'/imagens_iti/'.$obj_del->imagem_gde))
            {
                unlink(UPLOAD_DIR.'/imagens_iti/'.$obj_del->imagem_gde);
            }    
        }
    ...
    case 'editar':
        ...
        $cod_secoes = validaVarPost('cod_secoes');
        ...
        if ($codigo <= 0)
        {
            $sql_edicao = sprintf("INSERT INTO $tabela (cod_secoes, titulo, texto, data_texto, orientacao_imagem, situacao, link, data_hora_texto) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', NOW())", $cod_secoes, $titulo, $texto, $data_texto, $orientacao_imagem, $situacao, $link);
        ...
        {
            $sql_edicao = sprintf("UPDATE $tabela SET cod_secoes = '%s', titulo = '%s', texto = '%s', data_texto = '%s', orientacao_imagem = '%s', situacao = '%s', link = '%s' WHERE $chave_primaria = $codigo", $cod_secoes, $titulo, $texto, $data_texto, $orientacao_imagem, $situacao, $link);
            $res_edicao = mysql_query($sql_edicao);
        }
...
<link rel="stylesheet" type="text/css" media="screen" href="../../sys/lib/css/tabs_simples.css" />
...
function excluirImagem(cod) {
  if (confirm('Deseja excluir esta imagem?\n\nATENÇÃO: Este é um processo irreversível.')) {
    ...
    if(cod_textos> 0) {
      var url = 'acao=' + acao + '&cod_textos=' + cod_textos;
      
      new Request.JSON({url: 'iti_texto_ajax.php', onComplete: function(retorno) {
...
{
	if (document.frmIncluir.cod_secoes.value == 4)
	{
		document.getElementById("link").style.visibility='visible';
		document.getElementById("lbllink").style.visibility='visible';
	}
	else
	{
		document.getElementById("link").value = "";
		document.getElementById("link").style.visibility='hidden';
		document.getElementById("lbllink").style.visibility='hidden';
	}	
}
...
<? endif; ?>
        ...
        <table align="center" class="caixa" cellpadding="0" cellspacing="0">
			...
			<tr>
		        ...
		        <td class="legenda tdbr tdbt" colspan="2">
		        <select name="cod_secoes">
		            <option value=""></option>
		                
		                <?
		                $conexao = conectabd();
		                
		                $sql_sessoes = "SELECT * FROM iti_secoes ORDER BY secao";
		                $res_sessoes = mysql_query($sql_sessoes);
		                
		                while ($obj_sessoes = mysql_fetch_object($res_sessoes))
		                {
		                    echo '<option value="' . $obj_sessoes->cod_secoes . '" ';
		                    
		                    if ($obj_sessoes->cod_secoes == $cod_secoes)
		                    {
		                        echo 'selected="selected"';
		                    }
		                    
		                    echo '>' . bd2texto($obj_sessoes->secao) . '</option>';
		                }
            ...
            <tr>
                ...
                <td>&nbsp;</td>
                <td class=" tdbr"><input type="text" name="filtro" size="60" value="<? echo $filtro?>"></td>
            </tr>
        ...
        if ($cod_secoes>0)
        {
        	$sessao = 'AND s.cod_secoes ='.$cod_secoes;
        }
        
        $conexao = conectabd();
        
        $sql_buscar_registros = "SELECT * FROM $tabela t INNER JOIN iti_secoes s ON(t.cod_secoes=s.cod_secoes) WHERE t.$opcoes LIKE '%$filtro%' $sessao ";
        ...
        for ($b = 0; $b < $numpag; $b++)
        {
            ...
            echo '<input type="hidden" name="cod_secoes" value="' . $cod_secoes . '">';
...
<table align="center" class="caixa" cellpadding="0" cellspacing="0">
    <tr>
        <td class="legenda tdbl tdbt tdbr" colspan="2"><label for="cod_secoes" class="requerido">Seção</label></td>
    </tr>
    <tr>
        <td class="tdbl tdbr sep">
        
          <select name="cod_secoes" class="requerido" id="cod_secoes" onchange="javascript: verificar_link();">
              <option value=""></option>
              <?
              $conexao = conectabd();
              $sql_sessoes = "SELECT * FROM iti_secoes ORDER BY secao";
              ...
              while ($obj_sessoes = mysql_fetch_object($res_sessoes))
              {
                  ...
                  if ($obj_sessoes->cod_secoes == $obj_editar->cod_secoes)
                  {
                      echo 'selected="selected"';
                  }
                  echo '>' . bd2texto($obj_sessoes->secao) . '</option>';
    ...
    <tr>
        <td class="legenda tdbl tdbr"><label class="requerido" for="titulo">Titulo</label></td>
    </tr>
    <tr>
...
if ($codigo > 0)
{
    echo '<script>verificar_link();</script>';
}
?>

<input type="hidden" name="acao" value="editar"> <input type="hidden" name="<? echo $chave_primaria?>" id="<? echo $chave_primaria?>" value="<? echo $codigo?>">
</form>
