...
switch ($acao)
{
  ...
  $merchant_id = implode(',', $_POST['merchant_id']);
  $dados_extra = json_encode($_POST['dados_extra'],true);
  $ifood_ligado = validaVarPost('ifood_ligado');
  $token_loja = validaVarPost('token_loja');
  $nota_ativa = validaVarPost('nota_ativa');
  $email_nota_fiscal = validaVarPost('email_nota_fiscal');
...
<tr>
  <td class="legenda tdbl tdbr"><label for="inscricao_estadual">Token Loja <i>Esse token serve para o programa de impressão e outras integrações</i></label></td>
</tr>
<tr>
  <td class="tdbl tdbr">
    <?php 
    $token_loja = str_replace('/', '', crypt(date('Y-my-MdHis').uniqid()));
    echo !empty($objBusca->token_loja)?$objBusca->token_loja:"Token não gerado, atualize os dados cadastrais para gerar o Token"; 
    ?>
    <input type="hidden" name="token_loja" id="token_loja" value="<? echo !empty($objBusca->token_loja)?$objBusca->token_loja:$token_loja; ?>">
  </td>
</tr>
...
<tr>
  <td class="legenda tdbl tdbr"><br /><br /><label>Nota fiscal ativa</label></td>
</tr>
<tr>
  <td class="sep tdbl tdbr">
    <hr noshade="noshade" color="#1C4B93">
...
<tr>
  <td class="tdbl tdbr">
    <select name="nota_ativa" id="nota_ativa">
      <?php 
      if($objBusca->nota_ativa == 3){
        ?>
        <option value='1' name='1'>Nenhuma</option>
        <option name='2' value='2'>Homologacao</option>
        <option selected name='3' value='3'>Produção</option>
        <?php
      }else if($objBusca->nota_ativa == 2){
        ?>
        <option name='1'value='1'>Nenhuma</option>
        <option selected name='2' value='2'>Homologacao</option>
        <option name='3' value='3'>Produção</option>
        <?php
      }else{
        ?>
        <option selected name='1' value='1'>Nenhuma</option>
        <option name='2' value='2'>Homologacao</option>
        <option name='3' value='3'>Produção</option>
        <?php
      }
...
<tr>
  <td class="legenda tdbl tdbr"><br /><br /><label>E-mail de backup da nota <i>*E-mail que será enviado toda nota fiscal</i></label></td>
</tr>
<tr>
  <td class="sep tdbl tdbr">
    <hr noshade="noshade" color="#1C4B93">
...
<tr>
  <td class="legenda tdbl tdbr"><label for="email_backup">Informe o e-mail de backup</label></td>
</tr>
<tr>
  <td class="tdbl tdbr">
...
<tr>
  <td class="legenda tdbl tdbr"><label for="ifood_ligado">iFood Ligado</label></td>
</tr>
<tr>
  <td class="tdbl tdbr">
    <select name="ifood_ligado" id="ifood_ligado">
      <?php  
      if($objBusca->ifood_ligado == 1){
        ?>
...
<tr>
  <?php 
  if(!empty($objBusca->merchant_id)){
  $merchant = explode(',',$objBusca->merchant_id);
  ?>
  <td class="tdbl tdbr" style="float: left;">
    <input class='floatLeft' type="text" name="merchant_id[]" value="<?php echo $merchant[0]; ?>">
    <button style="float: right;width: 27px;" id="addMerchant">+</button>
  </td>
  </tr>
  <?php
  unset($merchant[0]);
  foreach($merchant as $im=>$vm){
    ?>
  <tr>
  <td class="tdbl tdbr" style="float: left;">
    <input class='floatLeft' type="text" name="merchant_id[]" value="<?php echo $vm; ?>">
    <span style=" color: red; margin-left: 10px; font-weight: 600;cursor:pointer; " onclick="removeMerchant(this)" class="removeMerchant">X<span>
  </td>
  </tr>
  <?php
  }
...
<tr>
  <td class="tdbl tdbr" style="float: left;">
    <input class='floatLeft' type="text" name="merchant_id[]" value="">
    <button style="float: right;width: 27px;" id="addMerchant">+</button>
  </td>
</tr>
  <?php 
...
<script type="text/javascript">
  ...
  let newMerchant = '<tr><td class="tdbl tdbr" style="float: left;"><input type="text" name="merchant_id[]" value="" style="float: left;" /><span style=" color: red; margin-left: 10px; font-weight: 600;cursor:pointer; " onclick="removeMerchant(this)" class="removeMerchant">X<span></td></tr>';
  addMerchant.onclick = function(e){
    e.preventDefault();
    this.parentNode.parentNode.insertAdjacentHTML('afterend',newMerchant);
  }
  function removeMerchant(v){
