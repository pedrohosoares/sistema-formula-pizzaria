...
cabecalho('Cadastro de Mesas');

$acao = validaVarPost('acao');

$chave_primaria = 'cod_mesas';
$tabela = 'ipi_mesas';
$campo_ordenacao = 'codigo_cliente_mesa';
$campo_filtro_padrao = 'codigo_cliente_mesa';
$quant_pagina = 50;
...
switch ($acao)
{
    ...
    case 'editar':
        ...
        $cod_mesas_situacoes = validaVarPost('cod_mesas_situacoes');
        $codigo_cliente_mesa  = validaVarPost('codigo_cliente_mesa');
        ...
        if ($codigo <= 0)
        {
            $sql_edicao = sprintf("INSERT INTO $tabela (cod_mesas_situacoes,codigo_cliente_mesa,cod_pizzarias) VALUES (%d,'%s',%d)", $cod_mesas_situacoes,$codigo_cliente_mesa,$cod_pizzarias);
        ...
        {
            $sql_edicao = sprintf("UPDATE $tabela SET codigo_cliente_mesa  = '%s', cod_mesas_situacoes = %d, cod_pizzarias = %d WHERE $chave_primaria = $codigo", $codigo_cliente_mesa , $cod_mesas_situacoes,$cod_pizzarias);
            $res_edicao = mysql_query($sql_edicao);
        }
...
window.addEvent('domready', function(){
  ...
  tabs.addEvent('change', function(indice){
    if(indice == 1) {
        ...
        document.frmIncluir.codigo_mesa.value = '';
        document.frmIncluir.cod_mesas_situacoes.value = '';
...
<? endif; ?>
        ...
        $sql_buscar_registros = "SELECT t.*, ms.situacao, p.nome FROM $tabela t left join ipi_mesas_situacoes ms on ms.cod_mesas_situacoes = t.cod_mesas_situacoes inner join ipi_pizzarias p on p.cod_pizzarias = t.cod_pizzarias WHERE t.$opcoes LIKE '%$filtro%' and p.cod_pizzarias IN(".implode(',',$_SESSION['usuario']['cod_pizzarias']).") ";
        ...
        $sql_buscar_registros .= ' ORDER BY codigo_cliente_mesa LIMIT ' . ($quant_pagina * $pagina) . ', ' . $quant_pagina;
        $res_buscar_registros = mysql_query($sql_buscar_registros);
        ...
        <table class="listaEdicao" cellpadding="0" cellspacing="0"
            ...
            <thead>
                <tr>
                    ...
                    <td align="center">Código da Mesa</td>
            ...
            while ($obj_buscar_registros = mysql_fetch_object($res_buscar_registros))
            {
              ...
              echo '<td align="center"><a href="#" onclick="editar(' . $obj_buscar_registros->$chave_primaria . ')">' . bd2texto($obj_buscar_registros->codigo_cliente_mesa) . '</a></td>';
...
<table align="center" class="caixa" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <tr>
        <td class="legenda tdbl tdbr"><label class="requerido" for="codigo_cliente_mesa">Código da Mesa</label></td>
    </tr>
    <tr>
        <td class="tdbl tdbr"><input class="requerido" type="text" name="codigo_cliente_mesa" id="codigo_cliente_mesa" maxlength="100" size="45" value="<? echo bd2texto($obj_editar->codigo_cliente_mesa)?>"></td>
    </tr>
    
    <tr>
        <td class="legenda tdbr tdbl"><label for="cod_mesas_situacoes">Situação</label></td>
    </tr>
    <tr>
        <td class="tdbl tdbr" colspan="2">
            <select name="cod_mesas_situacoes">
            <?   
              $con = conectabd();
              $sql_buscar_situacoes = "SELECT * FROM ipi_mesas_situacoes";
              $res_buscar_situacoes = mysql_query($sql_buscar_situacoes);
              while($obj_buscar_situacoes = mysql_fetch_object($res_buscar_situacoes))
              {
                echo '<option value="'.$obj_buscar_situacoes->cod_mesas_situacoes.'"';
                if($obj_editar->cod_mesas_situacoes==$obj_buscar_situacoes->cod_mesas_situacoes)
                {
                   echo ' selected="selected"';
                }
                echo '>'.$obj_buscar_situacoes->situacao.'</option>';
              }
              desconectabd($con);
            ?>
                <!-- <option value="ATIVO" <? if($obj_editar->cod_mesas_situacoes == 'ATIVO') echo 'SELECTED' ?>>Ativo</option>  
                <option value="INATIVO" <? if($obj_editar->cod_mesas_situacoes == 'INATIVO') echo 'SELECTED' ?>>Inativo</option> -->
            </select>
        </td>
    </tr>
