...
cabecalho('Preços de Bordas por '.TIPO_EMPRESA);
...
switch ($acao)
{
    case 'editar' :
        ...
        $con = conectabd();
                ...
                if($cod_pizzaria_filt)
		              ...
		              if (is_array($tamanho))
		              {
		                  $SqlDelTamanho = "DELETE FROM ipi_tamanhos_ipi_bordas WHERE $chave_primaria = $codigo and cod_pizzarias = $cod_pizzaria";
		                  ...
		                  if (is_array($tamanho_checkbox))
		                  {
		                      for($t = 0; $t < count($tamanho); $t++)
		                      {
		                          if (in_array($tamanho[$t], $tamanho_checkbox))
		                          {
		                              ...
		                              $selecao = ($selecao_padrao[$tamanho[$t]] > 0) ? $selecao_padrao[$tamanho[$t]] : 0;
                                  $valor_imposto_ajustado = (moeda2bd($valor_imposto[$t]) > 0) ? moeda2bd($valor_imposto[$t]) : 0;

		                              $SqlEdicaoTamanho = sprintf("INSERT INTO ipi_tamanhos_ipi_bordas (cod_bordas, cod_tamanhos, cod_pizzarias, preco, pontos_fidelidade, selecao_padrao_borda, valor_imposto) VALUES ('%d', '%d', '%d', '%s', '%d', %d, '%s')", $codigo, $tamanho[$t], $cod_pizzaria, $cor_preco, $cor_fidelidade, $selecao, $valor_imposto_ajustado);
    ...
    case 'copiar_precos' :
    		...
    		$con = conectar_bd();
    		$SqlDelTamanho = "DELETE FROM ipi_tamanhos_ipi_bordas WHERE cod_pizzarias = $cod_copiar";
        ...
        $res_adicionar_preco = TRUE;
				if($resDelTamanho)
				{    		
  				$sql_pegar_precos = "select * from ipi_tamanhos_ipi_bordas where cod_pizzarias = $cod_matriz";
    			$res_pegar_precos = mysql_query($sql_pegar_precos);
    			
    			while($obj_pegar_precos = mysql_fetch_object($res_pegar_precos))
    			{
    					$sql_adicionar_preco = "insert into ipi_tamanhos_ipi_bordas(cod_bordas,cod_tamanhos,cod_pizzarias,preco,pontos_fidelidade, selecao_padrao_borda, valor_imposto) values (".$obj_pegar_precos->cod_bordas.",".$obj_pegar_precos->cod_tamanhos.",".$cod_copiar.",".$obj_pegar_precos->preco.",".$obj_pegar_precos->pontos_fidelidade.",".$obj_pegar_precos->selecao_padrao_borda.",".$obj_pegar_precos->valor_imposto.")";
...
<? if($cod_pizzaria_filt !=""): ?>
  <table class="listaEdicao" cellpadding="0" cellspacing="0">
          ...
          $SqlBuscaBordas = "SELECT t.* FROM $tabela t inner join ipi_bordas b on b.cod_bordas = t.cod_bordas ORDER BY borda ";// WHERE cod_pizzarias = ".validaVarPost('cod_pizzarias_filt')." 
          //echo $SqlBuscaPizzas;
          ...
          while ( $objBuscaBordas = mysql_fetch_object($resBuscaBordas) )
          {
             	...
             	while($obj_tam = mysql_fetch_object($res_tam))
              {
  		          ...
  		          $sql_precos = "select tp.preco,t.tamanho,t.cod_tamanhos,tp.selecao_padrao_borda from ipi_tamanhos_ipi_bordas tp inner join ipi_tamanhos t on t.cod_tamanhos = tp.cod_tamanhos where tp.cod_bordas = ".$objBuscaBordas->cod_bordas." and tp.cod_tamanhos = ".$obj_tam->cod_tamanhos." and tp.cod_pizzarias = '$cod_pizzaria_filt' order by cod_tamanhos";
  		          $res_precos = mysql_query($sql_precos);
  		          $obj_precos = mysql_fetch_object($res_precos);
  	          	echo '<td align="center">'.($obj_precos->preco ?'R$' :'' ).' '.bd2moeda($obj_precos->preco).'&nbsp; '.($obj_precos->selecao_padrao_borda ?' (Padrão)' : '' ).'</td>';
...
<div class="painelTab">
	...
	<tr>
		<td class="legenda tdbl tdbr tdbt"><label 
			for="borda">Borda</label></td>
	</tr>
	<tr>
		<td class="tdbl tdbr"><input type="text"
			name="borda" id="borda" maxlength="45" size="45"
			value="<?
echo texto2bd($objBusca->borda)?>" readonly></td>
	</tr>
	...
	<tr>
		...
		<table class="listaEdicao" cellpadding="0" cellspacing="0">
          ...
          <td align="center"><label>Preço <br />Fidelidade (pontos)</label></td>
        ...
        while ( $objBuscaTamanhos = mysql_fetch_object($resBuscaTamanhos) )
        {
            ...
            if ($codigo > 0)
                $objBuscaPrecosTamanho = executaBuscaSimples(sprintf("SELECT * FROM ipi_tamanhos_ipi_bordas WHERE cod_bordas = '%d' AND cod_tamanhos = '%d' and cod_pizzarias= '%d' order by cod_tamanhos", $codigo, $objBuscaTamanhos->cod_tamanhos,$cod_pizzaria_filt), $con);
            else
            ...
            echo '<td align="center"><input type="text" name="fidelidade[]" id="fidelidade_' . $objBuscaTamanhos->cod_tamanhos . '" maxsize="5" size="3" value="' . $objBuscaPrecosTamanho->pontos_fidelidade . '" onKeyPress="return ApenasNumero(event)"></td>';
            //echo '<td align="center"><input type="text" name="estoque[]" id="estoque' . $objBuscaTamanhos->cod_tamanhos . '" maxsize="5" size="3" value="' . $objBuscaPrecosTamanho->quantidade_estoque_borda . '" onKeyPress="return ApenasNumero(event)"></td>';
            ...
            echo '<td align="center"><input type="checkbox" name="selecao['.$objBuscaTamanhos->cod_tamanhos.']" id="selecao_' . $objBuscaTamanhos->cod_tamanhos . '" value="1" '.($objBuscaPrecosTamanho->selecao_padrao_borda ? 'checked="checked"' : '').' ></td>';
            echo '</tr>';
        }
