...
switch ($acao)
{
	...
	if ($obj_buscar_caixa->situacao == "ABERTO")
	{
		...
		if($obj_buscar_pizzaria->cod_pizzarias > 0)
		{
			...
			$sql_abrir_caixa = sprintf("INSERT INTO ipi_caixa (cod_pizzarias, cod_usuarios_abertura, cod_usuarios_fechamento, data_hora_abertura, situacao) VALUES ('%s', '%s', '%s', '".DATA_HORA."', 'ABERTO')", $obj_buscar_pizzaria->cod_pizzarias, $_SESSION['usuario']['codigo'], $_SESSION['usuario']['codigo']);
...
<script>
	function fechar_caixa()
	{
		...
		let fechar_caixa_mesmo = document.querySelector("#fechar_caixa_mesmo");
		let baixar_abertos = document.querySelector("#baixar_abertos");
		let dados_fechamento = document.querySelector('p#dados_fechamento');
		let total_pedidos_abertos = document.querySelector('td.total_pedidos_abertos span');
		let valorTotalAberto = parseFloat(total_pedidos_abertos.innerText);
		...
		dados_fechamento.innerHTML = "<br /><strong>Total de pedidos sem baixar:</strong> R$"+total_pedidos_abertos.innerText;
		dados_fechamento.innerHTML +="<br />";

		if(valorTotalAberto >0){
			modal.setAttribute('style','display: block;position: absolute;top: 0;width: 100%;background: rgb(255,255,255,.7);height: 100%;left: 0;');
		}else{
			validaSubmit();
		}

		fechar_caixa_mesmo.onclick = function(e){
			e.preventDefault();	
			modal.setStyle('display','none');
			validaSubmit();
		}
		baixar_abertos.onclick = function(e){
			e.preventDefault();
			modal.setStyle('display','none');			
		}
	}
		...
		if($cod_pizzarias > 0):
			...
			<tr>
				<td class="tdbl tdbr set">
					...
					$rel_fechamento .= '<td style="background-color: #e5e5e5;" align="center" width="100"><b>IFOOD</b></td>';
					$rel_fechamento .= '<td style="background-color: #e5e5e5;" align="center" width="100"><b>TELEFONE</b></td>';
					$rel_fechamento .= '<td style="background-color: #e5e5e5;" align="center" width="100"><b>SITE</b></td>';
					$rel_fechamento .= '<td style="background-color: #e5e5e5;" align="center" width="100"><b>Total</b></td>';
					$rel_fechamento .= '</tr>';

					$total_geral_forma_pg_mesa = 0;
					$total_geral_forma_pg_loja = 0;
					$total_geral_forma_pg_ifood = 0;
					$total_geral_forma_pg_telefone = 0;
					$total_geral_forma_pg_site = 0;
					$total_geral_forma_pg = 0;

					$sqlTotalMesa = "SELECT SUM(pfg.valor) AS soma_mesa FROM ipi_pedidos p LEFT JOIN ipi_pedidos_formas_pg pfg ON (pfg.cod_pedidos = p.cod_pedidos) WHERE p.data_hora_pedido BETWEEN '$data_inicial' AND '$data_final' AND p.origem_pedido = 'MESA' AND p.situacao = 'BAIXADO' AND p.cod_pizzarias = $cod_pizzarias;";
					$sqlTotalTel = "SELECT SUM(pfg.valor) AS soma_mesa FROM ipi_pedidos p LEFT JOIN ipi_pedidos_formas_pg pfg ON (pfg.cod_pedidos = p.cod_pedidos) WHERE p.data_hora_pedido BETWEEN '$data_inicial' AND '$data_final' AND p.origem_pedido = 'TEL' AND p.situacao = 'BAIXADO' AND p.cod_pizzarias = $cod_pizzarias;";
					$sqlTotalIfood = "SELECT SUM(pfg.valor) AS soma_mesa FROM ipi_pedidos p LEFT JOIN ipi_pedidos_formas_pg pfg ON (pfg.cod_pedidos = p.cod_pedidos) WHERE p.data_hora_pedido BETWEEN '$data_inicial' AND '$data_final' AND p.origem_pedido = 'IFOOD' AND p.situacao = 'BAIXADO' AND p.cod_pizzarias = $cod_pizzarias;";
					$sqlTotalNet = "SELECT SUM(pfg.valor) AS soma_mesa FROM ipi_pedidos p LEFT JOIN ipi_pedidos_formas_pg pfg ON (pfg.cod_pedidos = p.cod_pedidos) WHERE p.data_hora_pedido BETWEEN '$data_inicial' AND '$data_final' AND p.origem_pedido = 'NET' AND p.situacao = 'BAIXADO' AND p.cod_pizzarias = $cod_pizzarias;";

					$sqlTodos = "SELECT c.origem_pedido,pa.forma_pg,p.valor FROM ipi_pedidos c 
					LEFT JOIN ipi_pedidos_formas_pg p ON (c.cod_pedidos = p.cod_pedidos)
					LEFT JOIN ipi_formas_pg pa ON (p.cod_formas_pg = pa.cod_formas_pg)
					WHERE c.data_hora_pedido BETWEEN '$data_inicial' AND '".DATA_HORA."' AND c.cod_pizzarias='".$cod_pizzarias."' AND c.situacao='BAIXADO';";

					$sqlTodosNaoBaixado = "SELECT c.cod_pedidos,c.origem_pedido,pa.forma_pg,p.valor FROM ipi_pedidos c 
					LEFT JOIN ipi_pedidos_formas_pg p ON (c.cod_pedidos = p.cod_pedidos)
					LEFT JOIN ipi_formas_pg pa ON (p.cod_formas_pg = pa.cod_formas_pg)
					WHERE c.data_hora_pedido BETWEEN '$data_inicial' AND '".DATA_HORA."' AND c.cod_pizzarias='".$cod_pizzarias."' AND c.situacao NOT IN ('BAIXADO','CANCELADO');";
					$sqlTodosNaoBaixado = mysql_query($sqlTodosNaoBaixado);
					
					$sqlTodos = "SELECT c.origem_pedido,pa.forma_pg,p.valor FROM ipi_pedidos c 
					LEFT JOIN ipi_pedidos_formas_pg p ON (c.cod_pedidos = p.cod_pedidos)
					LEFT JOIN ipi_formas_pg pa ON (p.cod_formas_pg = pa.cod_formas_pg)
					WHERE c.data_hora_pedido BETWEEN '".$data_inicial."' AND '".DATA_HORA."' AND c.cod_pizzarias='".$cod_pizzarias."' AND c.situacao='BAIXADO';";
					$sqlTodos = mysql_query($sqlTodos);
					$numSqlTodos = mysql_num_rows($sqlTodos);
					if($numSqlTodos >0){
						while ($registros = mysql_fetch_assoc($sqlTodos)) {
							$meios[$registros['origem_pedido']][] = array('valor'=>$registros['valor'],'forma_pg'=>$registros['forma_pg']);
						}
					}
					if(isset($meios)){

						foreach($meios as $i=>$v){
							foreach($v as $index=>$valor){
								if(!isset($novoMeio[$i][$valor['forma_pg']])){
									$novoMeio[$i][$valor['forma_pg']] = 0;
								}
								$novoMeio[$i][$valor['forma_pg']] += $valor['valor'];
							}
						}
					}

					if(isset($novoMeio)){
						if(isset($novoMeio['IFOOD'])){
							foreach($novoMeio['IFOOD'] as $i=>$v){
								if(!isset($meioTabela[$i]['IFOOD'])){
									$meioTabela[$i]['IFOOD'] = 0;
								}
								$meioTabela[$i]['IFOOD'] += $v;  
							}
						}
						if(isset($novoMeio['TEL'])){
							foreach($novoMeio['TEL'] as $i=>$v){
								if(!isset($meioTabela[$i]['TEL'])){
									$meioTabela[$i]['TEL'] = 0;
								}
								$meioTabela[$i]['TEL'] += $v;  
							}
						}
						if(isset($novoMeio['NET'])){
							foreach($novoMeio['NET'] as $i=>$v){
								if(!isset($meioTabela[$i]['NET'])){
									$meioTabela[$i]['NET'] = 0;
								}
								$meioTabela[$i]['NET'] += $v;  
							}
						}
						if(isset($novoMeio['MESA'])){
							foreach($novoMeio['MESA'] as $i=>$v){
								if(!isset($meioTabela[$i]['MESA'])){
									$meioTabela[$i]['MESA'] = 0;
								}
								$meioTabela[$i]['MESA'] += $v;  
							}
					...
					$html = '';
					foreach($meioTabela as $i=>$v){

						$mesa = isset($v['MESA'])?($v['MESA']):0;
						$loja = isset($v['LOJA'])?($v['LOJA']):0;
						$ifood = isset($v['IFOOD'])?($v['IFOOD']):0;
						$tel = isset($v['TEL'])?($v['TEL']):0;
						$net = isset($v['NET'])?($v['NET']):0;
						$total  =$mesa+$loja+$ifood+$tel+$net;
						$html .="<tr>";
						$html .= "<td>".$i."</td>";
						$html .= "<td style='text-align:center' class='mesa'>R$".$mesa."</td>";
						$html .= "<td style='text-align:center' class='loja'>R$".$loja."</td>";
						$html .= "<td style='text-align:center' class='ifood'>R$".$ifood."</td>";
						$html .= "<td style='text-align:center;' class='telefone'>R$".$tel."</td>";
						$html .= "<td style='text-align:center' class='site'>R$".$net."</td>";
						$html .= "<td style='text-align:center' class='total'>R$".$total."</td>";
						$html .="</tr>";
						$total_geral_forma_pg_mesa +=$mesa;
						$total_geral_forma_pg_loja +=$loja;
						$total_geral_forma_pg_ifood +=$ifood;
						$total_geral_forma_pg_telefone +=$tel;
						$total_geral_forma_pg_site +=$net;
						$totalTudo = $total_geral_forma_pg_mesa+$total_geral_forma_pg_loja+$total_geral_forma_pg_ifood+$total_geral_forma_pg_telefone+$total_geral_forma_pg_site;
					}
					$rel_fechamento .= $html;
					$rel_fechamento .= '<tr>' . "\r\n";
					$rel_fechamento .= '<td style="background-color: #e5e5e5;" align="right"><b>Total:</b></td>' . "\r\n";
					$rel_fechamento .= '<td align="center">R$' . bd2moeda($total_geral_forma_pg_mesa) . '</td>' . "\r\n";
					$rel_fechamento .= '<td align="center">R$' . bd2moeda($total_geral_forma_pg_loja) . '</td>' . "\r\n";
					$rel_fechamento .= '<td align="center">R$' . bd2moeda($total_geral_forma_pg_ifood) . '</td>' . "\r\n";
					$rel_fechamento .= '<td align="center">R$' . bd2moeda($total_geral_forma_pg_telefone) . '</td>' . "\r\n";
					$rel_fechamento .= '<td align="center">R$' . bd2moeda($total_geral_forma_pg_site) . '</td>' . "\r\n";
					$rel_fechamento .= '<td align="center"><b>R$' . bd2moeda($totalTudo) . '</b></td>' . "\r\n";
					$rel_fechamento .= '</tr>';
			...
			?>	
			<tr>
				<td class="legenda tdbl tdbr"><label for="abertos">Pedidos em ABERTO</label></td>
			</tr>
			<tr>
				<?php
				if(mysql_num_rows($sqlTodosNaoBaixado) == 0){
					?>
					<td class="tdbl tdbr">
			...
			<?php
			while($abertos = mysql_fetch_assoc($sqlTodosNaoBaixado)){
				$total+=$abertos['valor'];
				?>
				<tr>
					<td class="tdbl tdbr pedidos-abertos">
						<span class="cod_pedido_aberto"><?php echo $abertos['cod_pedidos'] ?></span> - R$<span class="valor_pedido_aberto"><?php echo number_format($abertos['valor'],2); ?></span>
					</td>
				</tr>
				<?php
			}
			...
			<tr>
				<td class="legenda tdbl tdbr">
					<label for="_abertos">Total pedidos ABERTOS: </label>
				</td>
			</tr>
			<tr>
				<td class='tdbl tdbr total_pedidos_abertos'>R$<span><?php echo $total; ?></span></td>
			</tr>
			<tr>
			...
			<div id="modal" style="display: none;">
				...
				<hr>
				<p id="dados_fechamento"></p>
				<button id="fechar_caixa_mesmo" style="
				background: red;
				color: #FFF;
				padding: 10px;
				border: 0px;
				...
				position: absolute;
				float: left;
				">Fechar assim mesmo</button>
				<button id="baixar_abertos" style="
				...
				position: absolute;
				margin-left: 192px;
				">Vou despachar primeiro</button>
			</div>
		</div>
	</div>
	<script type="text/javascript">
		let dados = '<?php echo json_encode($novoMeio); ?>';
		console.log(dados);
	</script>           
	<? 
