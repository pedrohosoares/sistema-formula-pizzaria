...
if ($cc)
{
  ...
  $sql_verificar_clientes = "SELECT * FROM ipi_clientes c WHERE $chave_primaria = '$codigo' AND c.cod_clientes IN (SELECT p.cod_clientes FROM ipi_pedidos p WHERE p.cod_clientes=c.cod_clientes AND p.cod_pizzarias IN(".$cod_pizzarias_usuario."))";
  //echo "<Br>1: ".$sql_verificar_clientes;
  $res_verificar_clientes = mysql_query($sql_verificar_clientes);
  $num_verificar_clientes = mysql_num_rows($res_verificar_clientes);
  if ($num_verificar_clientes>0)
  {
    $obj_verificar_clientes = mysql_fetch_object($res_verificar_clientes);
    $acao = "detalhes";
  }
  else
  {
    echo "<br /><center><strong><font style='color: #FF0000; font-size: 14px;'>Erro! Cliente de outra franquia!</font></strong></center><br />";
  }
...
switch($acao)
{
    ...
    case 'detalhes':
      ...
      if ($vip!="")
      {
    	...
    	$obj_buscar_clientes = executaBuscaSimples("SELECT * FROM ipi_clientes c WHERE $chave_primaria = '$codigo' AND c.cod_clientes IN (SELECT p.cod_clientes FROM ipi_pedidos p WHERE p.cod_clientes=c.cod_clientes AND p.cod_pizzarias IN(".$cod_pizzarias_usuario."))", $conexao);
      //echo "<Br>AKi: "."SELECT * FROM ipi_clientes c WHERE $chave_primaria = '$codigo' AND c.cod_clientes IN (SELECT p.cod_clientes FROM ipi_pedidos p WHERE p.cod_clientes=c.cod_clientes AND p.cod_pizzarias IN(".$cod_pizzarias_usuario."))";
...
/*        		var bordas = new FusionCharts('../lib/swf/fusioncharts/doughnut2d.swf', 'grafico_bordas', 300, 300, 0, 0, 'ffffff', 0);
    	...
    	<div id="tabs_internas" style="width: 100%; min-width: 700px;">
    		...
    		<div class="painelTabInterno">
                <? 
                    $sql_buscar_qtd_pedidos = "SELECT ( SELECT count(p.cod_pedidos) FROM ipi_pedidos p LEFT JOIN ipi_entregadores e ON (p.cod_entregadores = e.cod_entregadores) INNER JOIN ipi_pizzarias pi ON(p.cod_pizzarias=pi.cod_pizzarias) WHERE cod_clientes = '$codigo') qtd_totais,( SELECT count(p.cod_pedidos)FROM ipi_pedidos p LEFT JOIN ipi_entregadores e ON (p.cod_entregadores = e.cod_entregadores) INNER JOIN ipi_pizzarias pi ON(p.cod_pizzarias=pi.cod_pizzarias) WHERE cod_clientes = '$codigo' AND p.situacao='BAIXADO') qtd_finalizados,( SELECT count(p.cod_pedidos)FROM ipi_pedidos p LEFT JOIN ipi_entregadores e ON (p.cod_entregadores = e.cod_entregadores) INNER JOIN ipi_pizzarias pi ON(p.cod_pizzarias=pi.cod_pizzarias) WHERE cod_clientes = '$codigo' AND p.situacao='CANCELADO') qtd_cancelados,( SELECT count(p.cod_pedidos)FROM ipi_pedidos p LEFT JOIN ipi_entregadores e ON (p.cod_entregadores = e.cod_entregadores) INNER JOIN ipi_pizzarias pi ON(p.cod_pizzarias=pi.cod_pizzarias) WHERE cod_clientes = '$codigo' AND p.situacao!='BAIXADO' AND p.situacao!='CANCELADO') qtd_outros";
        ...
        <tbody>
			...
			<div class="painelTabInterno" align="center">
				...
				<table width="800" align="center">
                    ...
                    <tr>
                        <td align="center"><br />Para trocar a senha do cliente clique no botão abaixo, que será exibida a nova senha e enviada por e-mail.<br /><br /><div id="divReset"><input type="button" class="botaoAzul" name="resetar" value="Trocar Senha" onclick="javascript: resetar_senha(<? echo $codigo; ?>);"></div></td>
                    </tr>               
...
<?
    ...
    else
    {
     ...
     //   $opcoes = $_SESSION['usuario']['opcao_busca'];
...
<table align="center" class="caixa" cellpadding="0" cellspacing="0">
	...
	<tr>
		<td class="legenda tdbl tdbt" align="right"><select name="opcoes">
			<option value="nome"
				<?
    if($opcoes == 'nome')
	...
	<tr>
		<td class="legenda tdbl" align="right"><label
			for="situacao_filtro">Situação:</label></td>
		<td class="">&nbsp;</td>
		<td class="tdbr"><select name="situacao_filtro">
			<option value="TODOS"
...
<?
    ...
    $SqlBuscaRegistros = "SELECT c.*, 
      ...
      FROM $tabela c ".$filtrar_facebook." RIGHT JOIN ipi_enderecos e ON (c.cod_clientes=e.cod_clientes)  WHERE $opcoes LIKE '%$filtro%' AND c.cod_clientes IN (SELECT p.cod_clientes FROM ipi_pedidos p WHERE p.cod_clientes=c.cod_clientes AND p.cod_pizzarias IN(".$cod_pizzarias_usuario."))";
...
<table class="listaEdicao" cellpadding="0" cellspacing="0">
	<thead>
		<tr>
			...
			<td align="center"><a
				href="ipi_clientes_franquia.php?ord_pedidos=<? echo $ord_pedidos; ?>">Num. Pedidos Concluidos</a></td>
			<td align="center"><a
				href="ipi_clientes_franquia.php?ord_total=<? echo $ord_total; ?>">Total Gasto</a></td>
			<td align="center"><a
				href="ipi_clientes_franquia.php?ord_datacompra=<? echo $ord_datacompra; ?>">Data Última Compra</a></td>
    ...
    for($a = 0; $a < $linhasBuscaRegistros; $a++)
    {
        ...
        echo '<td align="center"><a href="ipi_clientes_franquia.php?cc='.$objBuscaRegistros->cod_clientes.'" >' . bd2texto($objBuscaRegistros->nome) . '</a></td>';
