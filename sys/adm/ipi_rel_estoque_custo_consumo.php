...
cabecalho('Relatório de Custo do Consumo de Ingredientes por Período');
...
if ($acao == "buscar")
{
  ...
  */
    ...
    $arr_ultimos_precos = array();
    foreach ($ingredientes_consumidos as $key => $value )
	  {

      $obj_ingredientes = executaBuscaSimples(sprintf("SELECT * FROM ipi_titulos t INNER JOIN ipi_estoque_entrada_itens eei ON (t.cod_estoque_entrada = eei.cod_estoque_entrada) INNER JOIN ipi_estoque_entrada ee ON (eei.cod_estoque_entrada = ee.cod_estoque_entrada) WHERE eei.cod_ingredientes = %d AND ee.data_hota_entrada_estoque < '".$data_final."' AND t.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND t.cod_pizzarias = '$cod_pizzarias' ORDER BY ee.data_hota_entrada_estoque DESC LIMIT 1", $key), $con);

      $arr_ultimos_precos[$obj_ingredientes->cod_ingredientes]["preco_unitario"] = $obj_ingredientes->preco_unitario_entrada;
      $arr_ultimos_precos[$obj_ingredientes->cod_ingredientes]["quantidade_embalagem"] = $obj_ingredientes->quantidade_embalagem_entrada;
      $arr_ultimos_precos[$obj_ingredientes->cod_ingredientes]["data_compra"] = $obj_ingredientes->data_hota_entrada_estoque;

	  }
/*
    echo "<pre>";
    print_r($arr_ultimos_precos);
    echo "</pre>";
*/
    ?>
      ...
      <table class="listaEdicao" cellpadding="0" cellspacing="0" align="center">
        <thead>
          <tr>
            ...
            <td align="center" width="90">Data Entrada</td>
            <td align="center" width="90">Qtde Embalagem</td>
            <td align="center" width="90">Preço Unit.</td>
            <td align="center" width="90">Custo Consumo</td>
          </tr>
        </thead>
        ...
        foreach ($ingredientes_consumidos as $key => $value )
          ...
          $obj_ingredientes = executaBuscaSimples(sprintf("SELECT * FROM ipi_ingredientes i WHERE i.cod_ingredientes = %d", $key), $con);
				  ...
				  echo '<td align="center">';
				  echo bd2datahora($arr_ultimos_precos[$key]["data_compra"]);
				  echo '</td>';

				  echo '<td align="center">';
				  echo bd2moeda($arr_ultimos_precos[$key]["quantidade_embalagem"]);
				  echo '</td>';

				  echo '<td align="center">';
				  echo bd2moeda($arr_ultimos_precos[$key]["preco_unitario"]);
				  echo '</td>';

				  echo '<td align="center">';
          if ($arr_ultimos_precos[$key]["quantidade_embalagem"]>0)
          {
  				  echo bd2moeda( ($value*$arr_ultimos_precos[$key]["preco_unitario"])/$arr_ultimos_precos[$key]["quantidade_embalagem"] );
            $total_ingredientes += ($value*$arr_ultimos_precos[$key]["preco_unitario"])/$arr_ultimos_precos[$key]["quantidade_embalagem"];
          }
				  echo '</td>';
            ...
            <tr>
              <td colspan="5" align="right">
                <strong>Total gasto com ingredientes: </strong>
              </td>

              <td align="center">
                <? echo bd2moeda($total_ingredientes); ?>
              </td>
            </tr>
  ...
  */
    ...
    $arr_ultimos_precos_bebidas = array();
    foreach ($bebidas_consumidas as $key => $value )
	  {

      $obj_bebidas = executaBuscaSimples(sprintf("SELECT * FROM ipi_titulos t INNER JOIN ipi_estoque_entrada_itens eei ON (t.cod_estoque_entrada = eei.cod_estoque_entrada) INNER JOIN ipi_estoque_entrada ee ON (eei.cod_estoque_entrada = ee.cod_estoque_entrada) WHERE eei.tipo_entrada_estoque = 'BEBIDA' AND eei.cod_bebidas_ipi_conteudos = %d AND ee.data_hota_entrada_estoque < '".$data_final."' AND t.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND t.cod_pizzarias = '$cod_pizzarias' ORDER BY ee.data_hota_entrada_estoque DESC LIMIT 1", $key), $con);

      $arr_ultimos_precos_bebidas[$obj_bebidas->cod_bebidas_ipi_conteudos]["preco_unitario"] = $obj_bebidas->preco_unitario_entrada;
      $arr_ultimos_precos_bebidas[$obj_bebidas->cod_bebidas_ipi_conteudos]["quantidade_embalagem"] = $obj_bebidas->quantidade_embalagem_entrada;
      $arr_ultimos_precos_bebidas[$obj_bebidas->cod_bebidas_ipi_conteudos]["data_compra"] = $obj_bebidas->data_hota_entrada_estoque;
        ...
        foreach ($bebidas_consumidas as $key => $value )
          ...
          $obj_bebida = executaBuscaSimples(sprintf("SELECT * FROM ipi_bebidas_ipi_conteudos bc INNER JOIN ipi_bebidas b ON(bc.cod_bebidas=b.cod_bebidas) INNER JOIN ipi_conteudos c ON(bc.cod_conteudos=c.cod_conteudos) WHERE bc.cod_bebidas_ipi_conteudos = %d", $key), $con);
				  ...
				  echo '<td align="center">';
				  echo bd2datahora($arr_ultimos_precos_bebidas[$key]["data_compra"]);
				  echo '</td>';

				  echo '<td align="center">';
				  echo bd2moeda($arr_ultimos_precos_bebidas[$key]["quantidade_embalagem"]);
				  echo '</td>';

				  echo '<td align="center">';
				  echo bd2moeda($arr_ultimos_precos_bebidas[$key]["preco_unitario"]);
				  echo '</td>';

				  echo '<td align="center">';
          if ($arr_ultimos_precos_bebidas[$key]["quantidade_embalagem"]>0)
          {
  				  echo bd2moeda( ($value*$arr_ultimos_precos_bebidas[$key]["preco_unitario"])/$arr_ultimos_precos_bebidas[$key]["quantidade_embalagem"] );
            $total_bebidas += ($value*$arr_ultimos_precos_bebidas[$key]["preco_unitario"])/$arr_ultimos_precos_bebidas[$key]["quantidade_embalagem"];
          }
				  echo '</td>';
            ...
            <tr>
              <td colspan="5" align="right">
                <strong>Total gasto com bebidas: </strong>
              </td>


              <td align="center">
                <? echo bd2moeda($total_bebidas); ?>
              </td>
            </tr>
            ...
            <tr>
              ...
              <td align="center">
                <? echo bd2moeda($total_bebidas + $total_ingredientes); ?>
              </td>
            </tr>
