...
cabecalho('Consuta de Entradas de Estoque');
...
$campo_filtro_padrao = 'i.ingrediente';
$quant_pagina = 120;
...
<div class="menuTab">
<ul>
    <li><a style='display:none' href="javascript:;">Voltar lista</a></li>
...
<? endif; ?>
        ...
        <table align="center" class="caixa" cellpadding="0" cellspacing="0">
            ...
            <tr>
                <td class="legenda tdbl tdbt" align="right">
                  <select name="opcoes">
                        <option value="<? echo $campo_filtro_padrao ?>"<? if ($opcoes == $campo_filtro_padrao) {echo 'selected';}?>>Ingrediente</option>
            ...
            <!-- <tr>
              <td class="legenda tdbl sep" align="right"><label for="filtro_limites">Somente notas com items <br/>lançados fora dos limites?</label></td>
              <td class="">&nbsp;</td>
              <td class="tdbr sep">
        ...
        if($acao == 'buscar')
        {
          ...
          $sql_buscar_registros = "SELECT iee.data_hota_entrada_estoque,t.cod_titulos,t.numero_nota_fiscal, ifo.nome_fantasia, ifo.razao_social, t.cod_titulos,i.ingrediente,ieei.quantidade_embalagem_entrada,ieei.quantidade_entrada,up.divisor_comum,up.abreviatura,c.conteudo,b.bebida,ieei.preco_unitario_entrada,(SELECT SUM(valor) FROM ipi_titulos_parcelas itp WHERE itp.cod_titulos = t.cod_titulos) as total_nf  FROM $tabela t inner join ipi_fornecedores ifo ON (ifo.cod_fornecedores = t.cod_fornecedores) INNER JOIN ipi_estoque_entrada iee ON (iee.cod_estoque_entrada = t.cod_estoque_entrada) INNER JOIN ipi_estoque_entrada_itens ieei on ieei.cod_estoque_entrada = iee.cod_estoque_entrada LEFT JOIN ipi_ingredientes i on i.cod_ingredientes = ieei.cod_ingredientes LEFT JOIN ipi_bebidas_ipi_conteudos bc on bc.cod_bebidas_ipi_conteudos = ieei.cod_bebidas_ipi_conteudos LEFT JOIN ipi_unidade_padrao up on up.cod_unidade_padrao = i.cod_unidade_padrao LEFT JOIN ipi_bebidas b on b.cod_bebidas = bc.cod_bebidas LEFT JOIN ipi_conteudos c on c.cod_conteudos = bc.cod_conteudos WHERE $opcoes LIKE '%$filtro%' AND t.cod_pizzarias in(".$cod_pizzarias_usuario.",0) AND iee.data_hota_entrada_estoque BETWEEN '".$data_inicial." 00:00:00' AND '".$data_final." 23:59:59' ";
          ...
          $sql_buscar_registros .= ' ORDER BY b.bebida,i.ingrediente ASC LIMIT ' . ($quant_pagina * $pagina) . ', ' . $quant_pagina;//ultimo_coment desc,
          $res_buscar_registros = mysql_query($sql_buscar_registros);
        ...
        <table class="listaEdicao" cellpadding="0" cellspacing="0"
            ...
            <thead>
                <tr>
                    ...
                    <td align="center">Data e hora da Entrada</td>
                    <td align="center">Ingrediente</td>
                    <td align="center">Fornecedor<br />Nome Fantasia</td>
                    <td align="center">Quantidade Entrada</td>
                    <td align="center">Quantidade da Embalagem</td>
                    <td align="center">Total Entrado</td>
                    <td align="center">Valor Total do Ingrediente</td>
                    <td align="center">Valor Total da Nota</td>
                    <!-- <td align="center">Itens lançados fora dos limites</td> -->
                </tr>
            </thead>
            <tbody>
            <?//ieei.quantidade_embalagem_entrada,
            $a = 0;
            while ($obj_buscar_registros = mysql_fetch_object($res_buscar_registros))
            {
                echo '<tr>';
                
                if($obj_buscar_registros->divisor_comum>0)
                {
                  $divisor_comum = $obj_buscar_registros->divisor_comum;
                }
                else
                {
                  $divisor_comum = 1;
                ...
                echo '<td align="center">'.bd2datahora($obj_buscar_registros->data_hota_entrada_estoque).'</td>';
                //<a href="#" onclick="editar(' . $obj_buscar_registros->cod_titulos . ')"></a>

                echo '<td align="center" >';
                if($obj_buscar_registros->ingrediente!="")
                {
                  echo $obj_buscar_registros->ingrediente.' ('.$obj_buscar_registros->abreviatura.')</td>';
                }
                else
                {
                  echo $obj_buscar_registros->bebida.' - '.$obj_buscar_registros->conteudo.'</td>'; 
                }
                echo '<td align="center" >'.$obj_buscar_registros->nome_fantasia.'</td>';

                echo '<td align="center" >'.$obj_buscar_registros->quantidade_entrada.'</td>';
                echo '<td align="center" >'.bd2moeda($obj_buscar_registros->quantidade_embalagem_entrada/$divisor_comum).'</td>';
                echo '<td align="center" >'.bd2moeda(($obj_buscar_registros->quantidade_entrada*$obj_buscar_registros->quantidade_embalagem_entrada)/$divisor_comum).'</td>';
                echo '<td align="center" >R$ '.bd2moeda(($obj_buscar_registros->preco_unitario_entrada*$obj_buscar_registros->quantidade_entrada)).'</td>';
                
                echo '<td align="center" >R$ '.bd2moeda(($obj_buscar_registros->total_nf)*-1).'</td>';
...
<div class="painelTab">
    ...
    ?>
      <div style='width:1000px; margin:0 auto;'>
        ...
        <?
          $sql_buscar_itens_nf = "SELECT ieei.cod_estoque_entrada_itens,ieei.entrada_fora_limites, ieei.cod_bebidas_ipi_conteudos, ieei.cod_ingredientes, ieei.tipo_entrada_estoque, ieei.quantidade_entrada, ieei.quantidade_embalagem_entrada, ieei.preco_unitario_entradaFROM ipi_titulos it INNER JOIN ipi_estoque_entrada iee ON (iee.cod_estoque_entrada = it.cod_estoque_entrada) INNER JOIN ipi_estoque_entrada_itens ieei ON (ieei.cod_estoque_entrada = iee.cod_estoque_entrada) WHERE it.cod_titulos = '".$codigo."'";
          ...
          <tbody>
            <?
              while($obj_buscar_itens_nf = mysql_fetch_object($res_buscar_itens_nf))
              {
                ...
                {
                  $sql_buscar_produtos = "SELECT ii.ingrediente, up.abreviatura, up.divisor_comum, iim.ingrediente_marca FROM ipi_ingredientes ii LEFT JOIN ipi_unidade_padrao up ON (ii.cod_unidade_padrao = up.cod_unidade_padrao) WHERE ii.cod_ingredientes = '".$obj_buscar_itens_nf->cod_ingredientes."'";
                  $res_buscar_produtos = mysql_query($sql_buscar_produtos);
                  $obj_buscar_produtos = mysql_fetch_object($res_buscar_produtos);
                  $divisor_comum = ($obj_buscar_produtos->divisor_comum > 0 ? $obj_buscar_produtos->divisor_comum : 1);
        ...
        <table class='listaEdicao'>
          <thead>
            <tr>
              <td width='220'>ICMS Substituição</td>
              <td width='220'>Valor total do IPI</td>
              <td width='220'>Outras despesas Acessórias</td>
              <td width='340'>Total com impostos</td>
            </tr>
          </thead>  
