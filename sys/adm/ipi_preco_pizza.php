...
cabecalho('Preços de '.TIPO_PRODUTOS);
...
switch ($acao)
{
    case 'editar' :
        ...
        $con = conectabd();
                ...
                if($cod_pizzaria_filt)
		              ...
		              if (is_array($tamanho))
		              {
		                  $SqlDelTamanho = "DELETE FROM ipi_pizzas_ipi_tamanhos WHERE $chave_primaria = $codigo and cod_pizzarias = $cod_pizzaria";
		              ...
		              if ($resDelTamanho)
		              {
		                  ...
		                  if (is_array($tamanho_checkbox))
		                  {
		                      for($t = 0; $t < count($tamanho); $t++)
		                      {
		                          ...
		                          {
		                              $cor_preco = (moeda2bd($preco[$t]) > 0) ? moeda2bd($preco[$t]) : 0;
		                              $cor_fidelidade = ($fidelidade[$t] > 0) ? $fidelidade[$t] : 0;
                                  $cod_impressoras = ($cod_impressoras[$t] > 0) ? $cod_impressoras[$t] : 0;
                                  $valor_imposto_ajustado = (moeda2bd($valor_imposto[$t]) > 0) ? moeda2bd($valor_imposto[$t]) : 0;

		                              if ($cod_impressoras>0)
		                              {
                                    $SqlEdicaoTamanho = sprintf("INSERT INTO ipi_pizzas_ipi_tamanhos (cod_pizzas, cod_impressoras, cod_tamanhos,cod_pizzarias, preco, pontos_fidelidade, valor_imposto) VALUES (%d, %d, %d, %d, %s, %d, %s)", $codigo, $cod_impressoras, $tamanho[$t], $cod_pizzaria, $cor_preco, $cor_fidelidade, $valor_imposto_ajustado );
                                  }
                                  else
                                  {
                                    $SqlEdicaoTamanho = sprintf("INSERT INTO ipi_pizzas_ipi_tamanhos (cod_pizzas, cod_tamanhos,cod_pizzarias, preco, pontos_fidelidade, valor_imposto) VALUES (%d, %d, %d, %s, %d, %s)", $codigo, $tamanho[$t], $cod_pizzaria, $cor_preco, $cor_fidelidade, $valor_imposto_ajustado );
                                  }
    ...
    case 'copiar_precos' :
    		...
    		$con = conectar_bd();
    		$SqlDelTamanho = "DELETE FROM ipi_pizzas_ipi_tamanhos WHERE cod_pizzarias = $cod_copiar";
        ...
        $res_adicionar_preco = TRUE;
				if($resDelTamanho)
				{    		
  				$sql_pegar_precos = "select * from ipi_pizzas_ipi_tamanhos where cod_pizzarias = $cod_matriz";
    			$res_pegar_precos = mysql_query($sql_pegar_precos);
    			
    			while($obj_pegar_precos = mysql_fetch_object($res_pegar_precos))
    			{
    					$sql_adicionar_preco = "insert into ipi_pizzas_ipi_tamanhos(cod_pizzas,cod_tamanhos,cod_pizzarias,preco,pontos_fidelidade, valor_imposto) values (".$obj_pegar_precos->cod_pizzas.",".$obj_pegar_precos->cod_tamanhos.",".$cod_copiar.",".$obj_pegar_precos->preco.",".$obj_pegar_precos->pontos_fidelidade.",".$obj_pegar_precos->valor_imposto.")";
        ...
        case 'alterar_impressora':
          ...
          $sql_del = "UPDATE ipi_pizzas_ipi_tamanhos set cod_impressoras = '$cod_impressoras' WHERE $chave_primaria IN ($indices_sql) and cod_pizzarias = '$cod_pizzaria_filt'";
          // die($sql_del);
          ...
          {
              mensagemerro('Erro ao alterar os registros', 'Por favor, comunique a equipe de suporte informando todos os usuários selecionados para exclusão.');
          }
        ...
        case 'excluir_impressora':
          ...
          $sql_del = "UPDATE ipi_pizzas_ipi_tamanhos set cod_impressoras = NULL WHERE $chave_primaria IN ($indices_sql) and cod_pizzarias = '$cod_pizzaria_filt'";
          /*echo $sql_del;*/
          // die($sql_del);
          if (mysql_query($sql_del))
          {
...
function validar_impressora(frm)
{
  if (frm.cod_impressoras.value=="")
  {
    alert("Selecione uma Impressora!");
    frm.cod_impressoras.focus(); 
    return false;
  }
  return true;
}
...
<table>
	<tr>
		...
		<form name="frmExcluir" method="post" onsubmit="return validar_impressora(this);">
<? if($cod_pizzaria_filt > 0) : ?>
      ...
      <table class="listaEdicao" cellpadding="0" cellspacing="0">
          ...
          $SqlBuscaPizzas = "SELECT p.*, tp.tipo_pizza FROM $tabela p LEFT JOIN ipi_tipo_pizza tp ON (p.cod_tipo_pizza= tp.cod_tipo_pizza)  ORDER BY tp.tipo_pizza ASC, p.pizza ASC, p.codigo_cliente_pizza ASC";// WHERE cod_pizzarias = ".validaVarPost('cod_pizzarias_filt')." 
          //echo $SqlBuscaPizzas;
          ...
          while ( $objBuscaPizzas = mysql_fetch_object($resBuscaPizzas) )
          {
              ...
              echo '<td align="center"><a href="javascript:;" onclick="editar(' . $objBuscaPizzas->$chave_primaria . ')">' . bd2texto($objBuscaPizzas->codigo_cliente_pizza) . '</a></td>';
              echo '<td align="center"><a href="javascript:;" onclick="editar(' . $objBuscaPizzas->$chave_primaria . ')">' . bd2texto($objBuscaPizzas->tipo_pizza) . '</a></td>';
             	...
             	while($obj_tam = mysql_fetch_object($res_tam))
              {
  		          ...
  		          $sql_precos = "select tp.preco,t.tamanho,t.cod_tamanhos,ip.nome_impressora from ipi_pizzas_ipi_tamanhos tp inner join ipi_tamanhos t on t.cod_tamanhos = tp.cod_tamanhos left join ipi_impressoras ip on tp.cod_impressoras = ip.cod_impressoras where tp.cod_pizzas = ".$objBuscaPizzas->cod_pizzas." and tp.cod_tamanhos = ".$obj_tam->cod_tamanhos." and tp.cod_pizzarias = '$cod_pizzaria_filt' order by cod_tamanhos";
                //echo "<br><br>".$sql_precos;
  		          $res_precos = mysql_query($sql_precos);
  		          $obj_precos = mysql_fetch_object($res_precos);
  	          	echo '<td align="center">'.($obj_precos->preco ?'R$' :'' ).' '.bd2moeda($obj_precos->preco).'&nbsp; </td>';
                if ($obj_precos->nome_impressora!="")
                {
                  $impressora = $obj_precos->nome_impressora;
                }

              	if($obj_precos->preco!="") $i++;
              }
              echo '<td align="center">'.$impressora.'</td>';
              echo '</tr>';
          }
    ...
    <? endif; ?>
        ...
        <!-- <input type="hidden" name="acao" value="excluir_impressora"> -->
      ...
      <li><a href="ipi_preco_pizza.php?cod_filt=<? echo $cod_pizzaria_filt ;?>">Preço de <?php echo TIPO_PRODUTOS; ?></a></li>
		</ul>
		</div>
		</td>
		<!-- Barra Lateral -->
...
<div class="painelTab">
	...
	<tr>
		<td class="legenda tdbl tdbr tdbt"><label 
			for="pizza"><?php echo TIPO_PRODUTO; ?></label></td>
	</tr>
	<tr>
		<td class="tdbl tdbr"><input type="text"
			name="pizza" id="pizza" maxlength="45" size="45"
			value="<?
echo texto2bd($objBusca->pizza)?>" readonly></td>
	</tr>
	...
	<tr>
		...
		<table class="listaEdicao" cellpadding="0" cellspacing="0">
          ...
          <td align="center"><label>(%) Imposto</label></td>
				</tr>
			</thead>
			<tbody>
        ...
        while ( $objBuscaTamanhos = mysql_fetch_object($resBuscaTamanhos) )
        {
            ...
            if ($codigo > 0)
                $objBuscaPrecosTamanho = executaBuscaSimples(sprintf("SELECT * FROM ipi_pizzas_ipi_tamanhos WHERE cod_pizzas = %d AND cod_tamanhos = %d and cod_pizzarias= %d order by cod_tamanhos", $codigo, $objBuscaTamanhos->cod_tamanhos,$cod_pizzaria_filt), $con);
            else
                $objBuscaPrecosTamanho = null;
            
            echo '<input type="hidden" name="tamanho[]" value="' . $objBuscaTamanhos->cod_tamanhos . '">';
            echo '<input type="hidden" name="cod_impressoras[]" value="' . $objBuscaPrecosTamanho->cod_impressoras . '">';
            ...
            echo '<td align="center"><input type="text" name="valor_imposto[]" id="valor_imposto_' . $objBuscaTamanhos->cod_tamanhos . '" maxsize="5" size="6" value="' . bd2moeda($objBuscaPrecosTamanho->valor_imposto) . '" onKeyPress="return formataMoeda(this, \'.\', \',\', event)"></td>';
