...
cabecalho('Pontos de Fidelidade Manual');
...
switch($acao) 
{
	case 'cadastrar':
    	...
    	$pontos = validaVarPost('pontos');
    	$observacao = validaVarPost('observacao');
    	$cod_clientes = validaVarPost('cod_clientes');
    	$cod_pedidos = validaVarPost('cod_pedidos');

    	$sql_verificar_pontos_creditados_pedido = "SELECT cod_pedidos FROM ipi_fidelidade_clientes WHERE cod_pedidos = $cod_pedidos";
    	$res_verificar_pontos_creditados_pedido = mysql_query($sql_verificar_pontos_creditados_pedido);
    	$num_verificar_pontos_creditados_pedido = mysql_num_rows($res_verificar_pontos_creditados_pedido);

    	if ($num_verificar_pontos_creditados_pedido <=1){

	    	$sql_inserir_pontos = sprintf("INSERT INTO ipi_fidelidade_clientes (cod_clientes, cod_pedidos, cod_usuarios, pontos, obs, data_hora_fidelidade, data_validade) VALUES ('%s', '%s', '%s', '%s', '%s', NOW(), DATE_ADD(NOW(), INTERVAL 1 YEAR) )", $cod_clientes, $cod_pedidos, $cod_usuarios, $pontos, $observacao);
		...
		if($res_inserir_pontos) 
		{
    		...
    		mensagemErro('Erro ao creditar os pontos', 'Já foram adicionados os pontos para este pedido!');
    	}
...
if ($acao == "buscar")
{
    ...
    $con = conectabd();
    $sql_buscar = "SELECT p.*, c.cod_clientes, c.nome, pi.nome nome_pizzaria FROM ipi_pedidos p INNER JOIN ipi_clientes c ON (p.cod_clientes=c.cod_clientes) INNER JOIN ipi_pizzarias pi ON (p.cod_pizzarias=pi.cod_pizzarias) WHERE p.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND p.cod_pedidos = '$int_num_pedido' AND p.situacao = 'BAIXADO'";
            ...
            <hr noshade="noshade" color="#D44E08" size="1">
				...
				<table width="100%" border="0">
					<tr>
						...
						<td width="60%" align="left"><input type="text" name="pontos" id="pontos" value="<? echo (int)($obj_buscar->valor_total) ?>"></td>
					</tr>
					<tr>
                    ...
                    <tr>
                        <td align="right"><label id="usuario_inclusao">Usuário que está incluindo:</label></td>
                        <td align="left"><? echo $_SESSION['usuario']['usuario']; ?></td>
                    </tr>
                    <tr>
                        <td align="right"><label id="data_inclusao">Data Inclusão:</label></td>
                        <td align="left"><? echo date("d/m/Y"); ?></td>
                    </tr>
					<tr>
						<td colspan="2" align="center"><br /><input type="submit" name="bt_enviar" value="Adicionar Pontos" class="botao"></td>
					</tr>
				</table>
    ...
    else 
    {
		...
		<tr>
			<td colspan="2" align="center">O pedido não foi encontrado ou ainda não foi baixado! <b><? echo $num_pedido; ?></b></td>
		</tr>
	    <?
    }
}
