...
cabecalho('Relatório DRE por Inventários');
?>
...
function detalhes_contas(acao,nome_exibir,cod_pizzarias,pagina)
  ...
  function carregar_contagens(tipo,cod)
  {
    var opcoes = "method:'post'";//method:'post'  
    var variaveis = 'acao=carregar_contagens&cod='+cod+'&tipo='+tipo;
  ...
  {
    var c1 = document.getElementById('contagem_1');
    var c2 = document.getElementById('contagem_2');

    if (c1.selectedIndex == -1) return false;
    if (c2.selectedIndex == -1) return false;

    document.getElementById('data_inicial').value = c1.options[c1.selectedIndex].text;
    document.getElementById('data_final').value = c2.options[c2.selectedIndex].text;
...
$cod_inventario1 = validaVarPost("contagem_1");  
$cod_inventario2 = validaVarPost("contagem_2");
...
echo "<td align='left'> $sinal ".$arr_tcat[$cod_categoria]['nome_categoria']."</td>";
  ...
  echo '
    <form name="frmFiltroTeorico" method="post" action="ipi_rel_cmv_teorico.php" target="_blank">
      ...
      <input type="hidden" value="'.date("d/m/Y",strtotime($data_hora_contagem1)).'" name="data_inicial" id="data_inicial">

      <input type="hidden" value="'.date("d/m/Y",strtotime($data_hora_contagem2)).'" name="data_final" id="data_final">
...
<form name="frmFiltro" method="post" onsubmit="return verificar_datas()">
 ...
 <table align="center" class="caixa" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td class="tdbt">&nbsp;</td>
    <td class="tdbr tdbt">
      <select name="cod_pizzarias" id="cod_pizzarias" onchange="carregar_contagens('contagem_1',this.value)">
  ...
  <tr>
      <td class="legenda tdbl " align="right"><label for="contagem_1">Primeiro Inventário:</label></td>
      <td >&nbsp;</td>
      <td class="tdbr ">
        <select class="requerido" name="contagem_1" id="contagem_1" onchange="carregar_contagens('contagem_2',this.value)">
          <? if($cod_inventario1)
          {
            $sql_buscar_contagens = "select cod_inventarios,data_hora_contagem from ipi_estoque_inventario where cod_pizzarias = '".$cod_pizzarias."' and cod_pizzarias in ($cod_pizzarias_usuario) order by data_hora_contagem DESC";
            $res_buscar_contagens = mysql_query($sql_buscar_contagens);
            while($obj_buscar_contagens = mysql_fetch_object($res_buscar_contagens))
            {
                echo "<option value='".$obj_buscar_contagens->cod_inventarios."'";

                if($cod_inventario1==$obj_buscar_contagens->cod_inventarios)
                {
                  echo" selected='selected'";
                }

                echo">".bd2data(date("Y-m-d",strtotime($obj_buscar_contagens->data_hora_contagem)))." </option>";
            }
          }
          else
          {
            echo '<option value="">Selecione um(a) '.TIPO_EMPRESA.' !</option>';
          }

          ?>
          
          <?
            /*$sql_buscar_contagems = "SELECT * FROM ipi_estoque_inventario where cod_pizzarias in ($cod_pizzarias_usuario)";
            $res_buscar_contagems = mysql_query($sql_buscar_contagems);
            while($obj_buscar_contagems = mysql_fetch_object($res_buscar_contagems))
            {
              echo "<option value='".$obj_buscar_contagems->cod_inventarios."'";
              if($cod_inventario1==$obj_buscar_contagems->cod_inventarios)
              {
                echo "SELECTED = 'SELECTED' ";
              }

              echo ">'".$obj_buscar_contagems->cod_pizzarias."' - '".$obj_buscar_contagems->cod_inventarios."' - '".$obj_buscar_contagems->cod_usuarios_contagem."' - '".bd2datahora($obj_buscar_contagems->data_hora_contagem)."'</option>";
            }*/
  ...
  <tr>
    <td class="legenda tdbl " align="right"><label for="contagem_2">Último Inventário:</label></td>
    <td >&nbsp;</td>
    <td class="tdbr ">
      <select class="requerido" name="contagem_2" id="contagem_2">
        <? if($cod_inventario2)
          {
            $sql_buscar_contagens = "select cod_inventarios,data_hora_contagem from  ipi_estoque_inventario where cod_pizzarias = '".$cod_pizzarias."' and cod_inventarios > '".$cod_inventario1."' and cod_pizzarias in ($cod_pizzarias_usuario) order by data_hora_contagem DESC";
            $res_buscar_contagens = mysql_query($sql_buscar_contagens);
            while($obj_buscar_contagens = mysql_fetch_object($res_buscar_contagens))
            {
                echo "<option value='".$obj_buscar_contagens->cod_inventarios."'";

                if($cod_inventario2==$obj_buscar_contagens->cod_inventarios)
                {
                  echo" selected='selected'";
                }
          ...
          {
            echo '<option value="">Selecione o primeiro Inventário</option>';
          }
        ...
        <?
          ...
          {
            echo "<option value='".$obj_buscar_contagems->cod_inventarios."'";
            if($cod_inventario2==$obj_buscar_contagems->cod_inventarios)
            {
              echo "SELECTED = 'SELECTED' ";
            }
            echo ">'".$obj_buscar_contagems->cod_pizzarias."' - '".$obj_buscar_contagems->cod_inventarios."' - '".$obj_buscar_contagems->cod_usuarios_contagem."' - '".bd2datahora($obj_buscar_contagems->data_hora_contagem)."'</option>";
      ...
      <input type='hidden' name='data_inicial' id='data_inicial' value='<? echo date("Y-m-d") ?>'/>
      <input type='hidden' name='data_final' id='data_final' value='<? echo date("Y-m-d") ?>'/>
...
<!--    <tr>
  ...
  <tr><td align="center" class="tdbl tdbb tdbr" colspan="3"><input class="botaoAzul" type="submit" value="Buscar"><input type='hidden' name="filtrar_filtro" value="DATA_CRIADA"/><input type='hidden' name='acao' value='buscar' /></td></tr>
...
if($acao=="buscar"): ?>
        ...
        <?
        $conexao = conectabd();
        $sql_buscar_data_contagems = "SELECT (SELECT data_hora_contagem3 from ipi_estoque_contagem where cod_inventarios = '$cod_inventario1'  LIMIT 1) as data_contagem1,(SELECT data_hora_contagem3 from ipi_estoque_contagem where cod_inventarios = '$cod_inventario2' LIMIT 1) as data_contagem2";
        $res_buscar_data_contagems = mysql_query($sql_buscar_data_contagems);
        $obj_buscar_data_contagems = mysql_fetch_object($res_buscar_data_contagems);

        $data_inicial_filtro = date("Y-m-d",strtotime($obj_buscar_data_contagems->data_contagem1));
        $data_final_filtro = date("Y-m-d",strtotime("-1 day",strtotime($obj_buscar_data_contagems->data_contagem2)));//menos 1 pq os inventarios sao sempre feitos antes das pizzarias abrirem
        ...
        echo "<div style='display:none'>".$sql_buscar_plano_contas."</div>";
