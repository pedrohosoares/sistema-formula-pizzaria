...
cabecalho('Cadastro de Contas a Receber');
...
{
    case 'excluir':
        $file = 'log_alteracoes_titulos_receber.txt';
    ...
    case 'editar':
        ...
        if ($codigo <= 0)
        {
            $sql_edicao = sprintf("INSERT INTO $tabela (cod_pizzarias, cod_titulos_subcategorias, cod_colaboradores, cod_clientes, cod_entregadores, cod_fornecedores, total_parcelas, tipo_titulo, tipo_cedente_sacado, descricao, numero_nota_fiscal,data_hora_criacao) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', 'RECEBER', '%s', '%s', '%s',NOW())", 
                                $cod_pizzarias, $cod_titulos_subcategorias, $cod_colaboradores, $cod_clientes, $cod_entregadores, $cod_fornecedores, $num_parcelas, $tipo_cedente_sacado, $descricao, $numero_nota_fiscal);
            ...
            if(mysql_query($sql_edicao))
            {
                ...
                if($obj_buscar_titulos_subcategorias->num_parcelas_maximo == 1)
                {
                    ...
                    $sql_edicao_parcelas = sprintf("INSERT INTO ipi_titulos_parcelas (cod_titulos, data_vencimento,data_emissao, valor, juros, valor_total, numero_parcela, mes_ref, ano_ref, situacao,data_hora_criacao) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s','%s', '%s', '%s',NOW())",
                                            $codigo, data2bd($vencimento),data2bd($emissao), moeda2bd($valor), 0, moeda2bd($valor), 1, $mes_ref, $ano_ref, 'ABERTO');
                ...
                else if($obj_buscar_titulos_subcategorias->num_parcelas_maximo > 1)
                {
                    ...
                    if(is_array($cod_titulos_parcelas))
                    {
                        ...
                        for($i = 0; $i < count($cod_titulos_parcelas); $i++)
                        {
                            ...
                            $sql_edicao_parcelas = sprintf("INSERT INTO ipi_titulos_parcelas (cod_titulos, data_vencimento, data_emissao, valor, juros, valor_total, numero_parcela, mes_ref, ano_ref, situacao,data_hora_criacao) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s',NOW())",
                                                    $codigo, data2bd($vencimento[$i]),data2bd($emissao[$i]), moeda2bd($valor[$i]), 0, moeda2bd($valor[$i]), $i + 1, $mes_ref, $ano_ref, 'ABERTO');
        ...
        {
            $sql_edicao = sprintf("UPDATE $tabela SET cod_pizzarias = '%s', cod_titulos_subcategorias = '%s', cod_colaboradores = '%s', cod_clientes = '%s', cod_entregadores = '%s', cod_fornecedores = '%s', tipo_cedente_sacado = '%s', descricao = '%s', numero_nota_fiscal = '%s' WHERE $chave_primaria = $codigo", 
                            $cod_pizzarias, $cod_titulos_subcategorias, $cod_colaboradores, $cod_clientes, $cod_entregadores, $cod_fornecedores, $tipo_cedente_sacado, $descricao, $numero_nota_fiscal);
            // file_put_contents($file, "\n SQL ALTERA TITULO \n".$sql_edicao, FILE_APPEND);
            if(mysql_error($conexao)!="")
            {
                file_put_contents($file, "ERRO ".mysql_errno($conexao) . ": " . mysql_error($conexao) . "\n",FILE_APPEND);
            ...
            if(mysql_query($sql_edicao))
            {
                if($obj_buscar_titulos_subcategorias->num_parcelas_maximo == 1)
                {
                    ...
                    $sql_edicao_parcelas = sprintf("UPDATE ipi_titulos_parcelas SET data_vencimento = '%s', data_emissao = '%s',valor = '%s', valor_total = '%s', mes_ref = '%s', ano_ref = '%s' WHERE cod_titulos_parcelas = '" . $obj_buscar_parcelas->cod_titulos_parcelas . "'",
                                            data2bd($vencimento),data2bd($emissao), moeda2bd($valor), moeda2bd($valor), $mes_ref, $ano_ref);
                ...
                else if($obj_buscar_titulos_subcategorias->num_parcelas_maximo > 1)
                {
                    ...
                    for($i = 0; $i < count($cod_titulos_parcelas); $i++)
                    {
                        ...
                        $sql_edicao_parcelas = sprintf("UPDATE ipi_titulos_parcelas SET data_vencimento = '%s',data_emissao = '%s', valor = '%s', valor_total = '%s', mes_ref = '%s', ano_ref = '%s' WHERE cod_titulos_parcelas = '" . $cod_titulos_parcelas[$i] . "'",
                                            data2bd($vencimento[$i]), data2bd($emissao[$i]), moeda2bd($valor[$i]), moeda2bd($valor[$i]), $mes_ref, $ano_ref);
...
function criar_formulario_edicao(cod_titulos_subcategorias, cod_titulos)
{
    ...
    new Request.HTML(
    {
        url: 'ipi_titulos_receber_ajax.php',
...
window.addEvent('domready', function()
{
    ...
    tabs.addEvent('change', function(indice)
    {
        if(indice == 1)
        {
            ...
            document.frmIncluir.cod_clientes.value = '';
...
<? endif; ?>
        ...
        $data_inicial_filtro = (validaVarPost('data_inicial_filtro')) ? validaVarPost('data_inicial_filtro') : date("d/m/Y");
        $data_final_filtro = (validaVarPost('data_final_filtro')) ? validaVarPost('data_final_filtro') : date("d/m/Y");
        ...
        $cliente_filtro = validaVarPost('cliente_filtro');
        ...
        <table align="center" class="caixa" cellpadding="0" cellspacing="0">
            ...
            <tr>
                <td class="legenda tdbl sep " align="right"><label for="cliente_filtro">Cliente:</label></td>
                <td class=" ">&nbsp;</td>
                <td class="tdbr ">
                    <input type="text" name="cliente_filtro" id="cliente_filtro" size="50" value="<? echo $cliente_filtro ?>">
                </td>
            </tr>
        ...
        $conexao = conectabd();
         ...
         if ($tr){
                $codigo = $tr;
                $sql_buscar_registros = "SELECT DISTINCT t.*, pp.*, tc.*, tp.situacao, tp.valor, tp.mes_ref, tp.ano_ref, tp.situacao FROM $tabela t INNER JOIN ipi_titulos_parcelas tp ON (t.cod_titulos = tp.cod_titulos) INNER JOIN ipi_titulos_subcategorias pp ON (t.cod_titulos_subcategorias = pp.cod_titulos_subcategorias) INNER JOIN ipi_titulos_categorias tc ON (pp.cod_titulos_categorias = tc.cod_titulos_categorias) WHERE t.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND t.tipo_titulo = 'RECEBER' AND cod_titulos_parcelas = $codigo";
         }
         else{
                $sql_buscar_registros = "SELECT DISTINCT t.*, pp.*, tc.*, tp.situacao, tp.valor, tp.mes_ref, tp.ano_ref, tp.situacao FROM $tabela t INNER JOIN ipi_titulos_parcelas tp ON (t.cod_titulos = tp.cod_titulos) INNER JOIN ipi_titulos_subcategorias pp ON (t.cod_titulos_subcategorias = pp.cod_titulos_subcategorias) INNER JOIN ipi_titulos_categorias tc ON (pp.cod_titulos_categorias = tc.cod_titulos_categorias) ";

                if(trim($cliente_filtro) != '')
                {
                    $sql_buscar_registros .= " INNER JOIN ipi_clientes c ON (c.cod_clientes = t.cod_clientes) ";
                    $nome_filtro = " AND c.nome LIKE '%$cliente_filtro%' ";
                }

                $sql_buscar_registros .= "WHERE t.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND t.tipo_titulo = 'RECEBER' ";
        ...
        for ($b = 0; $b < $numpag; $b++)
        {
            ...
            echo '<input type="hidden" name="descricao_filtro" value="' . $descricao_filtro . '">';
             echo '<input type="hidden" name="sacado_filtro" value="' . $sacado_filtro . '">';
        ...
        <table class="listaEdicao" cellpadding="0" cellspacing="0" width="<?echo LARGURA_PADRAO?>">
            ...
            while ($obj_buscar_registros = mysql_fetch_object($res_buscar_registros))
            {
                ...
                if($obj_buscar_registros->tipo_cedente_sacado == 'CLIENTE')
                {
                	$obj_buscar_cedente = executaBuscaSimples("SELECT * FROM ipi_clientes WHERE cod_clientes = '" . $obj_buscar_registros->cod_clientes . "'", $conexao);

                	echo bd2texto($obj_buscar_cedente->nome);
                }
            	else if($obj_buscar_registros->tipo_cedente_sacado == 'FORNECEDOR')
                {
    ...
    <table align="center" class="caixa" cellpadding="0" cellspacing="0">
        ...
        <tr>
            <td class="legenda tdbl tdbt tdbr">
            	<label for="cod_titulos_subcategorias">Recebimento</label>
            </td>
        </tr>
       	<tr>
			<td class="tdbl tdbr sep">
            	<select name="cod_titulos_subcategorias" id="cod_titulos_subcategorias" style="width: 300px;" onchange="criar_formulario_edicao(this.value, document.frmIncluir.cod_titulos.value)">
                    ...
                    $sql_buscar_categorias = "SELECT * FROM ipi_titulos_categorias WHERE cod_titulos_categorias IN (SELECT cod_titulos_categorias FROM ipi_titulos_subcategorias WHERE tipo_titulo = 'RECEBER') ORDER BY titulos_categoria";
