...
cabecalho('Preços de Ingredientes Adicionais por '.TIPO_EMPRESA);
...
switch ($acao)
{
		                  ...
		                  if (is_array($tamanho_checkbox))
		                  {
		                      for($t = 0; $t < count($tamanho); $t++)
		                      {
                                  ...
                                  $valor_imposto_ajustado = (moeda2bd($valor_imposto[$t]) > 0) ? moeda2bd($valor_imposto[$t]) : 0;
		                              ...
		                              $SqlEdicaoTamanho = sprintf("INSERT INTO ipi_ingredientes_ipi_tamanhos (cod_ingredientes, cod_tamanhos,cod_pizzarias, preco, preco_troca, quantidade_estoque_extra, pontos_fidelidade, valor_imposto) VALUES (%d, %d, %d, %s, %s, %d, %d, %s)", $codigo, $tamanho[$t],$cod_pizzaria, $cor_preco, $cor_preco_troca, $cor_consumo, $cor_fidelidade, $valor_imposto_ajustado);
    ...
    case 'copiar_precos' :
        ...
        $res_adicionar_preco = true;
    			...
    			{
    					$sql_adicionar_preco = "insert into ipi_ingredientes_ipi_tamanhos(cod_ingredientes,cod_tamanhos,cod_pizzarias,preco,preco_troca,quantidade_estoque_extra, pontos_fidelidade, valor_imposto) values (".$obj_pegar_precos->cod_ingredientes.",".$obj_pegar_precos->cod_tamanhos.",".$cod_copiar.",".$obj_pegar_precos->preco.",".$obj_pegar_precos->preco_troca.",".$obj_pegar_precos->quantidade_estoque_extra.",".$obj_pegar_precos->valor_imposto.",".$obj_pegar_precos->pontos_fidelidade.")";
...
<table class="listaEdicao" cellpadding="0" cellspacing="0">
        ...
        $SqlBuscaIngredientes = "SELECT t.* FROM $tabela t where t.adicional =1 and t.consumo = 0 and t.ativo =1 GROUP BY t.ingrediente ORDER BY t.ingrediente ";// WHERE cod_pizzarias = ".validaVarPost('cod_pizzarias_filt')." 
  ...
  <? endif; ?>
      ...
      <li><a href="ipi_preco_pizza.php?cod_filt=<? echo $cod_pizzaria_filt ;?>">Preço de Pizzas</a></li>
		</ul>
		</div>
		</td>
		<!-- Barra Lateral -->
...
echo texto2bd($objBusca->ingrediente)?>" readonly></td>
	...
	<tr>
		...
		<table class="listaEdicao" cellpadding="0" cellspacing="0">
          ...
          <td align="center"><label>Preço <br/>Pontos <br />Fidelidade</label></td>
