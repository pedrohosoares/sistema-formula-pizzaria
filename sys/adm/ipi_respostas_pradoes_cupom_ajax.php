...
switch ($acao)
{
    case 'carregar_produto':
        ...
        if ($produto == 'PIZZA')
        {
            ...
            {
                echo '<option value="' . $objBuscaPizza->cod_pizzas . '"'.($objBuscaPizza->cod_pizzas==$cod_produtos? ' SELECTED ' : '').'>' . utf8_encode(bd2texto($objBuscaPizza->pizza)) . '</option>';
            }
        }
        else if ($produto == 'BORDA')
        {
            ...
            {
                echo '<option value="' . $objBuscaBorda->cod_bordas . '" '.($objBuscaBorda->cod_bordas==$cod_produtos? ' SELECTED ' : '').' >' . utf8_encode(bd2texto($objBuscaBorda->borda)) . '</option>';
            }
        }
        else if ($produto == 'BEBIDA')
        {
            ...
            {
                echo '<option value="' . $objBuscaBebidas->cod_bebidas_ipi_conteudos . '" '.($objBuscaBebidas->cod_bebidas_ipi_conteudos==$cod_produtos? ' SELECTED ' : '').' >' . utf8_encode(bd2texto($objBuscaBebidas->bebida . ' ' . $objBuscaBebidas->conteudo)) . '</option>';
            }
        }
    ...
    case 'carregar_tamanho':
        ...
        $produto = validaVarPost('produto');
        $cod_produtos = (validaVarPost('cod_produtos')) ? validaVarPost('cod_produtos') : 0;
        $cod_tamanhos = (validaVarPost('cod_tamanhos')) ? validaVarPost('cod_tamanhos') : 0;
        ...
        if ($cod_produtos <= 0)
        {
            ...
            {
                echo '<option value="' . $objBuscaTamanho->cod_tamanhos . '" '.($objBuscaTamanho->cod_tamanhos==$cod_tamanhos ? 'SELECTED' : '').'>' . utf8_encode(bd2texto($objBuscaTamanho->tamanho)) . '</option>';
            }
        }
