...
cabecalho('Registrar Entregas Avulsas');
...
switch ($acao)
{
    ...
    case 'editar':
        ...
        if ($codigo <= 0)
        {
            ...
            $sql_edicao = sprintf("INSERT INTO $tabela (cod_pizzarias, cod_entregadores, data_hora_entrega, bairro, cidade, estado,valor,cod_pedidos, obs_entrega_avulsa,tipo_entrega) VALUES ('%s', '%s', '%s', '%s', '%s', '%s','%s',%d, '%s','ENTREGA')", $cod_pizzarias, $cod_entregadores, $data_hora_entrega, $bairro,$cidade,$estado,moeda2bd($valor),$cod_pedidos, $obs_entrega_avulsa);
            //echo $sql_edicao;
...
<!--             <tr>
        ...
        $sql_buscar_registros .= " AND ea.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND ea.tipo_entrega = 'ENTREGA' and ea.data_hora_entrega BETWEEN '$data_inicial 00:00:00' AND '$data_final 23:59:59'";
        if($cod_pizzarias>0)
        {
...
-->
        <table class="listaEdicao" cellpadding="0" cellspacing="0"
            ...
            while ($obj_buscar_registros = mysql_fetch_object($res_buscar_registros))
            {
                ...
                echo '<td align="center">'.bd2texto($obj_buscar_registros->bairro)." - ".bd2texto($obj_buscar_registros->cidade)." - ".bd2texto($obj_buscar_registros->estado).'</td>';
                echo '<td align="center">'.bd2moeda($obj_buscar_registros->valor).'</td>';
                echo '<td align="center">'.bd2texto($obj_buscar_registros->cod_pedidos).'</td>';
...
<table align="center" class="caixa" cellpadding="0" cellspacing="0">
	...
	<tr>
		<td class="legenda tdbl tdbt tdbr"><label class="requerido"	for="cod_pizzarias"><?php echo TIPO_EMPRESA ?></label></td>
	</tr>
  <tr>
    ...
    <tr>
        <td class="legenda tdbl tdbr"><label class="requerido"
            for="valor">Valor</label></td>
    </tr>
    <tr>
        <td class="tdbl tdbr sep"><input class="requerido" type="text"
            name="valor" id="valor" maxlength="5" size="8" onkeypress="return formataMoeda(this, '.', ',', event);"></td>
    </tr>

    <tr>
        <td class="legenda tdbl tdbr"><label
            for="cod_pedidos">Pedido</label></td>
    </tr>
    <tr>
        <td class="tdbl tdbr sep"><input type="text"
            name="cod_pedidos" id="cod_pedidos" maxlength="8" size="8" onkeypress="return ApenasNumero(event);"></td>
    </tr>
