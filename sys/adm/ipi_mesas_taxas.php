...
cabecalho('Cadastro de Taxas');

$acao = validaVarPost('acao');

$chave_primaria = 'cod_mesas_taxas';
$tabela = 'ipi_mesas_taxas';
$campo_ordenacao = 'taxa';
$campo_filtro_padrao = 'taxa';
$quant_pagina = 50;
...
switch ($acao)
{
    ...
    case 'editar':
        ...
        $taxa = validaVarPost('taxa');
        $situacao  = validaVarPost('situacao');
        $tipo_taxa  = validaVarPost('tipo_taxa');
        $conexao = conectabd();
        
            
        if ($codigo <= 0)
        {
            $sql_edicao = sprintf("INSERT INTO $tabela (taxa,situacao,tipo_taxa) VALUES ('%s','%s','%s')", $taxa,$situacao,$tipo_taxa);
        ...
        {
            $sql_edicao = sprintf("UPDATE $tabela SET situacao  = '%s', taxa = '%s', tipo_taxa = '%s' WHERE $chave_primaria = $codigo", $situacao , $taxa,$tipo_taxa);
            $res_edicao = mysql_query($sql_edicao);
        }
...
window.addEvent('domready', function(){
  ...
  tabs.addEvent('change', function(indice){
    if(indice == 1) {
        ...
        document.frmIncluir.taxa.value = '';
        document.frmIncluir.situacao.value = '';
        document.frmIncluir.tipo_taxa.value = '';
...
<? endif; ?>
        ...
        $sql_buscar_registros .= ' ORDER BY taxa LIMIT ' . ($quant_pagina * $pagina) . ', ' . $quant_pagina;
        $res_buscar_registros = mysql_query($sql_buscar_registros);
        ...
        <table class="listaEdicao" cellpadding="0" cellspacing="0"
            ...
            <thead>
                <tr>
                    ...
                    <td align="center" >Tipo Taxa</td>
                    <td align="center" >Situação</td><!-- width="70"
width="50"-->
                </tr>
            </thead>
            <tbody>
            ...
            while ($obj_buscar_registros = mysql_fetch_object($res_buscar_registros))
            {
                ...
                echo '<td align="center"><a href="#" onclick="editar(' . $obj_buscar_registros->$chave_primaria . ')">' . bd2texto($obj_buscar_registros->taxa) . '</a></td>';

                echo '<td align="center">'. bd2texto($obj_buscar_registros->tipo_taxa).'</td>';
...
<table align="center" class="caixa" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td class="legenda tdbl tdbt tdbr"><label class="requerido" for="taxa">Taxa</label></td>
    </tr>
    <tr>
        <td class="tdbl tdbr"><input class="requerido" type="text" name="taxa" id="taxa" maxlength="100" size="45" value="<? echo bd2texto($obj_editar->taxa)?>"></td>
    </tr>
    
    <tr>
        <td class="legenda tdbr tdbl"><label for="tipo_taxa">Tipo de Taxa</label></td>
    </tr>
    <tr>
        <td class="tdbl tdbr" colspan="2">
            <select name="tipo_taxa">
                <option value="Valor" <? if($obj_editar->tipo_taxa == 'VALOR') echo 'SELECTED' ?>>Valor</option>  
                <option value="Percentual" <? if($obj_editar->tipo_taxa == 'PERCENTUAL') echo 'SELECTED' ?>>Percentual</option>
            </select>
        </td>
    </tr>
