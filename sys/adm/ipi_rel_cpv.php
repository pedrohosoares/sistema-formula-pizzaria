...
{
  var url = "acao=carregar_pizzas&cod_pizzarias="+<? echo ($acao == 'buscar' ? 'pizzaria' : 'pizzaria.getSelected().get("value")' );?>+"&cod_pizzas="+pizza;
  new Request.HTML(
  {
    url: 'ipi_rel_cpv_ajax.php',
    update: $('combo_pizza'),
    method:'post',
...
<div>
  ...
  <tr>
    ...
    <td class="tdbr tdbt sep">
      <select name="cod_pizzarias" id="cod_pizzarias" onChange="carregar_pizzas(this, <? echo $cod_pizzas; ?>)">
  ...
  <?
    if ( ($cod_pizzarias) || ($mes) )
    {
      ...
      while($obj_buscar_tamanhos = mysql_fetch_object($res_buscar_tamanhos))
      {
        ...
        $sql_buscar_receita = "SELECT ii.cod_ingredientes, ii.ingrediente, iie.quantidade_estoque_ingrediente FROM ipi_ingredientes ii INNER JOIN ipi_ingredientes_estoque iie ON (iie.cod_ingredientes = ii.cod_ingredientes) WHERE iie.cod_tamanhos = '".$cod_tamanhos."' AND iie.cod_pizzas = '".$cod_pizzas."'";
      ...
      {
        echo '<tr>';
        $sql_preco_ingrediente = "SELECT (preco_unitario_entrada/quantidade_embalagem_entrada) as preco_unidade, preco_unitario_entrada, (quantidade_embalagem_entrada*1000) as qtd_embalagem, iee.data_hota_entrada_estoque FROM ipi_estoque_entrada iee INNER JOIN ipi_estoque_entrada_itens ieei ON (iee.cod_estoque_entrada = ieei.cod_estoque_entrada) WHERE iee.data_hota_entrada_estoque < '".date("Y")."-".date("m", strtotime('+1 month'))."-01' AND ieei.cod_ingredientes = '".$indice."' ORDER BY iee.data_hota_entrada_estoque DESC LIMIT 1";
        ...
        $arr_valores[$indice]['Preco un'] = ($obj_preco_ingrediente->preco_unidade ? $obj_preco_ingrediente->preco_unidade : 0);
        $arr_valores[$indice]['qtd_embalagem'] = ($obj_preco_ingrediente->qtd_embalagem ? $obj_preco_ingrediente->qtd_embalagem : 0);
        ...
        $arr_valores[$indice]['preco_grama'] = ($arr_valores[$indice]['preco_unitario_entrada']/1000);
        
        echo '<td>'.$arr_valores[$indice]['ingrediente'].'</td>';
        //print_r($v_ingre);
      ...
      echo 'Selecione a pizzaria e o sabor!';
  ?>
  </div>
</div>
