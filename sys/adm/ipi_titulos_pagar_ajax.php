...
switch ($acao)
{
    case 'criar_formulario_edicao':
            ...
            <td class="sep">
            	<select name="cod_colaboradores" id="cod_colaboradores" style="width: 300px;">
                	...
                	while($obj_buscar_tipo_colaboradores = mysql_fetch_object($res_buscar_tipo_colaboradores))
                	{
                    	...
                    	$sql_buscar_colaboradores = "SELECT * FROM ipi_colaboradores WHERE cod_tipo_colaboradores = '" . $obj_buscar_tipo_colaboradores->cod_tipo_colaboradores . "' AND situacao = 'ATIVO' AND cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") ORDER BY nome";
    	...
    	<tr>
            <td class="sep">
            	<input type="text" name="vencimento" id="vencimento" maxlength="10" size="10" value="<? echo ($obj_titulos_parcelas->cod_titulos_parcelas > 0) ? bd2data($obj_titulos_parcelas->data_vencimento) : date('d/m/Y'); ?>" onkeypress="return MascaraData(this, event)">
            	&nbsp;
                ...
                <input type="text" name="valor" id="valor" size="15" style="margin-left: 10px;" value="<? echo bd2moeda($obj_titulos_parcelas->valor * -1) ?>" onkeypress="return formataMoeda(this, '.', ',', event)">
        ...
        new vlaDatePicker('emissao', {openWith: 'botao_emissao', prefillDate: false});</script>
    ...
    case 'criar_parcelas':
        ...
        <label>Parcela <? echo $i; echo ($obj_buscar_titulos_parcelas->situacao=="PAGO" ? '<span style="color:red;font-weight:bold"> - PAGA</span>' : ''); ?></label>
        <?if($obj_buscar_titulos_parcelas->situacao=="PAGO")
        {
    	...
    	<tr>
            <td class="sep">
            	<input type="text" name="vencimento[]" id="vencimento<? echo $i ?>" <? echo $readonly; ?> maxlength="10" size="10" value="<? echo bd2data($obj_buscar_titulos_parcelas->data_vencimento); ?>" <? if($readonly =='') echo 'onkeypress="return MascaraData(this, event)"' ;?>>
            	&nbsp;
            	<a href="javascript:;" <? echo $readonly; ?> id="botao_vencimento<? echo $i ?>"><img src="../lib/img/principal/botao-data.gif"></a>
            	...
            	<input type="text" name="valor[]" id="valor" <? echo $readonly; ?> size="15" style="margin-left: 10px;" value="<? echo bd2moeda($obj_buscar_titulos_parcelas->valor * -1) ?>" <? if($readonly =='') echo 'onkeypress="return formataMoeda(this, \'.\', \',\', event)"'; ?>>
            	<input type="text" name="mes_ref[]" id="mes_ref" <? echo $readonly; ?> size="11" style="margin-left: 10px;" value="<?  echo (($obj_buscar_titulos_parcelas->mes_ref != '') && ($obj_buscar_titulos_parcelas->ano_ref != '')) ? sprintf('%02d', $obj_buscar_titulos_parcelas->mes_ref) . '/' . $obj_buscar_titulos_parcelas->ano_ref : ''; ?>" <? if($readonly =='') echo 'onkeypress="return Mascara(this, event, \'##/####\')"'; ?>>
            </td>
        </tr>
