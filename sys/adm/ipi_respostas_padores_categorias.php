...
cabecalho('Cadastro Categorias para Respostas');

$acao = validaVarPost('acao');

$chave_primaria = 'cod_respostas_categorias';
$tabela = 'ipi_respostas_categorias';
...
switch ($acao)
{
    ...
    case 'editar':
        ...
        if ($codigo <= 0)
        {
            $sql_edicao = sprintf("INSERT INTO $tabela (cod_enquete_perguntas,nome_categoria,situacao) VALUES (%d,'%s','%s')", $nome_categoria,$situacao);
        ...
        {
            $sql_edicao = sprintf("UPDATE $tabela SET cod_enquete_perguntas = %d , nome_categoria = '%s', situacao = '%s' WHERE $chave_primaria = $codigo", $cod_enquetes, $nome_categoria, $situacao);
            $res_edicao = mysql_query($sql_edicao);
        }
...
<table align="center" class="caixa" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td class="tdbl tdbr sep"><input class="requerido" type="text" name="nome_categoria" id="nome_categoria" maxlength="100" size="45" value="<? echo bd2texto($obj_editar->nome_categoria)?>"></td>
    </tr>
    
    <tr>
        <td class="legenda tdbl tdbr"><label class="requerido" for="cod_enquetes">Pergunta da categoria</label></td>
    </tr>
    <tr>
        <td class="tdbl tdbr sep">
            <select name="cod_enquetes" id="cod_enquetes" class="requerido">
                <option vlaue=""></option>
                <?
                  $con = conectar_bd();
                  $sql_selecionar_perguntas = "select * from ipi_enquete_perguntas where pergunta_pessoal = 0";
                  $res_selecionar_perguntas = mysql_query($sql_selecionar_perguntas);
                  while($obj_selecionar_perguntas = mysql_fetch_object($res_selecionar_perguntas))
                  {
                    echo "<option value='".$obj_selecionar_perguntas->cod_enquete_perguntas."'".($obj_selecionar_perguntas->cod_enquete_perguntas==$obj_editar->cod_enquete_perguntas ? ' SELECTED' : '' )." >".$obj_selecionar_perguntas->pergunta."</option>";
                  }
                  desconectabd($con);
                ?>
            </select>
