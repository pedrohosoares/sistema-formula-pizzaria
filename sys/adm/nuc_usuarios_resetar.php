...
*/
    ...
    if ($ResDel1)
    {
      ...
      $SqlUpdateLog = sprintf('INSERT INTO nuc_usuarios_log (cod_usuarios_alterado,cod_usuarios_alterou,alteracao,ip_alteracao,data_hora_alteracao) (SELECT cod_usuarios,"%s","%s","%s",NOW() FROM nuc_usuarios where cod_usuarios in ('.$indicesSql.'))',$_SESSION['usuario']['codigo'],"Cadastrou usuario no sys",$ip);
  ...
  case 'editar':
    ...
    $cod_perfis   = validaVarPost('cod_perfis'); //Campo fixo com valor do PERFIL
    ...
    else 
    {
    	if ($cadastro_senha!="")
    	{	
      		$sql_edicao = sprintf("UPDATE nuc_usuarios SET usuario = '%s', nome = '%s', email = '%s', cod_perfis = %d, senha = MD5('%s'), situacao = '%s' WHERE cod_usuarios = %d", $cadastro_usuario, $nome, $email,$cod_perfis, $cadastro_senha, $situacao, $cod_usuarios);

          $ip = $_SERVER["REMOTE_ADDR"];
          $SqlUpdateLog = sprintf('INSERT INTO nuc_usuarios_log (cod_usuarios_alterado,cod_usuarios_alterou,alteracao,ip_alteracao,data_hora_alteracao) values("%s","%s","%s","%s",NOW())',$cod_usuarios,$_SESSION['usuario']['codigo'],"Alterou usuario e senha no sys",$ip);
          $ResUpdateLog = mysql_query($SqlUpdateLog);
    	}
    	else 
    	{
        ...
        $ip = $_SERVER["REMOTE_ADDR"];
        $SqlUpdateLog = sprintf('INSERT INTO nuc_usuarios_log (cod_usuarios_alterado,cod_usuarios_alterou,alteracao,ip_alteracao,data_hora_alteracao) values("%s","%s","%s","%s",NOW())',$cod_usuarios,$_SESSION['usuario']['codigo'],"Alterou usuario no sys",$ip);
        $ResUpdateLog = mysql_query($SqlUpdateLog);
    ...
    <td class="conteudo">
      ...
      <form name="frmExcluir" method="post" onsubmit="return verificaCheckbox(this)">
        ...
        <table class="listaEdicao" cellpadding="0" cellspacing="0">
          ...
          $SqlBuscaUsuarios = "SELECT * FROM nuc_usuarios u LEFT JOIN ipi_pizzarias_nuc_usuarios pu ON (u.cod_usuarios = pu.cod_usuarios) LEFT JOIN nuc_perfis g ON (u.cod_perfis = g.cod_perfis) WHERE pu.cod_pizzarias IN (".implode(',',$_SESSION['usuario']['cod_pizzarias']).") AND u.cod_perfis NOT IN (1, 2) AND u.cod_usuarios NOT IN (1, 2, 13, 15) AND u.situacao!='EXCLUIDO' ORDER BY u.usuario";
...
?>
    ...
    <tr><td class="legenda tdbl tdbr"><label class="requerido" for="cadastro_senha">Senha</label></td></tr>
  	<tr><td class="sep tdbl tdbr"><input type="text" name="cadastro_senha" id="cadastro_senha" maxlength="45" size="30"></td></tr>
    ...
    <tr>
      <td class="sep tdbl tdbr">
        <select class="requerido" name="cod_perfis" id="cod_perfis">
            <option value=""></option>
            <? 
            $sql_buscar_perfis = "SELECT * from nuc_perfis where cod_perfis in ('$cod_atendente','$cod_operacoes','$cod_atend_ope')";
            $res_buscar_perfis = mysql_query($sql_buscar_perfis);
            while($obj_buscar_perfis = mysql_fetch_object($res_buscar_perfis))
            {
              echo "<option value='".$obj_buscar_perfis->cod_perfis."'";
              if($objBusca->cod_perfis == $obj_buscar_perfis->cod_perfis) echo 'selected'; 
              echo ">".$obj_buscar_perfis->perfil."</option>";
            }
