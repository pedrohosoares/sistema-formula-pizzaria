...
cabecalho('Cadastro Motivos Gargalo de Eficiência');
...
switch ($acao)
{
    ...
    case 'editar':
        ...
        if ($codigo <= 0)
        {
            $sql_edicao = sprintf("INSERT INTO $tabela (motivo,situacao) VALUES ('%s', '%s')", $motivo,$status);
        ...
        {
            $sql_edicao = sprintf("UPDATE $tabela SET motivo = '%s', situacao = '%s' WHERE $chave_primaria = $codigo", $motivo, $status);
            $res_edicao = mysql_query($sql_edicao);
        }
...
window.addEvent('domready', function(){
  ...
  tabs.addEvent('change', function(indice){
    if(indice == 1) {
        ...
        document.frmIncluir.motivo.value = '';
...
<? endif; ?>
        ...
        $sql_buscar_registros .= ' ORDER BY t.cod_motivos DESC LIMIT ' . ($quant_pagina * $pagina) . ', ' . $quant_pagina;
        $res_buscar_registros = mysql_query($sql_buscar_registros);
        ...
        <table class="listaEdicao" cellpadding="0" cellspacing="0"
            ...
            <thead>
                <tr>
                    ...
                    <td align="center" width="30">Nº Motivo</td>
