...
<? endif; ?>
       ...
       /* if($opcoes!="")
        ...
        if($pizzaria)
            $sql_buscar_registros .=" and ctp.cod_pizzarias in('".$pizzaria."')";
       ...
       //   $ordenacao = "t.titulo_ticket";
        ...
        <table class="listaEdicao" cellpadding="0" cellspacing="0"
            ...
            <thead>
                <tr>
                  <? //echo ($ordenacao=="titulo" ? ($ordenacao_cresc=="ASC" ? ' class="ordem_asc"' : ' class="ordem_desc"') : '') ?>
                    ...
                    <td align="center" width="5%">Respostas</td>
            ...
            while ($obj_buscar_registros = mysql_fetch_object($res_buscar_registros))
            {
              ...
              echo '<td align="center" style="background-color:'.$cod_fundo_comentario.'" ><a href="?cc='.$obj_buscar_registros->cod_tickets.''.$filts_get.' ">' .sprintf("%04d",$obj_buscar_registros->cod_tickets)."-". bd2texto($obj_buscar_registros->titulo_ticket) . '</a><br/><span style="font-size:10px; ;">Categoria:'.$obj_buscar_registros->nome_subcategoria.'</span></td>';
...
if ($codigo > 0)
{
  ...
  if((($obj_editar->cod_usuarios==$codigo_usuario) || ($codigo_usuario==1) || ($codigo_perfil==4) || ($codigo_perfil==2)) && ($obj_editar->cod_situacoes!=2))
  {
    $pode_editar = true;
  }
  ...
  <form name="frmIncluir" method='post' action='' enctype="multipart/form-data" onsubmit="return validar_form_editar(this)">
    <table width="800px" align='center' class='detalhesTicket'>
      ...
      <tr>
          ...
          {
          echo '<label for="tempo_trabalhado">Tempo Trabalho</label><br/>';
      ...
      <? if($pode_editar && !$editar): ?>
        <tr>
          <td>
            ...
            <h3 style='color:#EB8612'>Ações</h3>
            <br/>
            ...
            <? if(($codigo_usuario==$obj_editar->cod_usuarios) ||  ( $pode_editar && ( $codigo_usuario==1 || $codigo_perfil==4 || $codigo_perfil==2) )): ?>
            &nbsp;&nbsp;&nbsp;&nbsp;
