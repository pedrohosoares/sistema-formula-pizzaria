...
cabecalho('Captura de Pagmentos VISANET MOSET dos Pedidos');
...
{
    window.open("visa_retorno_cancelar2.php?cod="+cod, "Pedido"+cod, "height = 300, width = 400");
}

function capturar(cod)
{
	window.open("visa_retorno_captura2.php?cod="+cod, "Pedido"+cod, "height = 300, width = 400");
}
...
<form name="frmCaptura" method="post">
  ...
  <table class="listaEdicao" cellpadding="0" cellspacing="0">
    ...
    $SqlBuscaPedidos = "SELECT pi.nome nome_pizzaria, p.*,c.* FROM $tabela p INNER JOIN ipi_clientes c ON (p.cod_clientes = c.cod_clientes) INNER JOIN ipi_pizzarias pi ON (p.cod_pizzarias = pi.cod_pizzarias) WHERE ((p.forma_pg='VISANET')OR(p.forma_pg='MASTERCARDNET')) AND p.situacao = 'IMPRESSO' AND p.cod_pizzarias > 0 AND p.origem_pedido = 'NET' AND p.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ")";
    if ($cod_pizzarias)
    ...
    while ($objBuscaPedidos = mysql_fetch_object($resBuscaPedidos))
    {
        ...
        echo '<td align="center"><input type="button" class="botaoVermelho" name="btCancelar" value="CANCELAR" onClick="if(confirm(\'Deseja realmente cancelar o pedido '.sprintf('%08d', $objBuscaPedidos->$chave_primaria).'?\')){cancelar('.$objBuscaPedidos->$chave_primaria.')}"></td>';
