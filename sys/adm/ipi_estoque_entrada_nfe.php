...
function criar_formulario_pagamento(cod_titulos_subcategorias)
{
    ...
    new Request.HTML(
    {
        url: 'ipi_estoque_entrada_nfe_ajax.php',
...
function carregar_ingrediente(cod_ingredientes,indice) {
  var iframe = document.getElementById('upload_nfe');
  var frameDoc = iframe.contentDocument || iframe.contentWindow.document;

  //var cep = document.frmCadastro.cep.value;
  var url = 'acao=carregar_ingredientes_marcas&cim=' + cod_ingredientes;
  
  if(cod_ingredientes != '') 
  {
    new Request.JSON({url: 'ipi_estoque_entrada_nfe_ajax.php', onComplete: function(retorno) {
      if(retorno.status == 'OK') {
      
        frameDoc.getElementById('cod_ingredientes_'+indice).value = cod_ingredientes;
        //frameDoc.getElementById('quantidade_ingredientes_adicionar_' + indice).value = retorno.bairro;

        if(frameDoc.getElementById('unidade_compra_nota_'+indice).value == "KG")
        {
          if(retorno.abreviatura=='kg')
          {
            frameDoc.getElementById('quantidade_ingredientes_adicionar_' + indice).value = 1;
            if(frameDoc.getElementById('unidade_ing_sys_' + indice).innerHTML==' g')
            {
              frameDoc.getElementById('quantidade_ingredientes_adicionar_embalagem_' + indice).value = frameDoc.getElementById('quantidade_ingredientes_adicionar_embalagem_' + indice).value.replace(",",".") /1000;
              frameDoc.getElementById('quantidade_ingredientes_adicionar_embalagem_' + indice).value = frameDoc.getElementById('quantidade_ingredientes_adicionar_embalagem_' + indice).value.replace(".",",");
            }
          }
          else
          {
        ...
        {
          frameDoc.getElementById('quantidade_ingredientes_adicionar_embalagem_' + indice).value = retorno.quantidade_emb;
        }

        //frameDoc.getElementById('preco_total_ingredientes_adicionar_' + indice).value = retorno.estado;
        frameDoc.getElementById('tipo_unidade_' + indice).value = retorno.abreviatura;
        frameDoc.getElementById('divisor_comum_' + indice).value = retorno.divisor_comum;
        frameDoc.getElementById('msg_erro_' + indice).style.display =  'none';
        frameDoc.getElementById('botao_add_' + indice).style.display =  'block';
        frameDoc.getElementById('entrada_estoque_minima_' + indice).value =  retorno.entrada_estoque_minima;
        frameDoc.getElementById('entrada_estoque_maxima_' + indice).value =  retorno.entrada_estoque_maxima;
        frameDoc.getElementById('unidade_ing_sys_' + indice).innerHTML = ' '+retorno.abreviatura;
       // document.frmCadastro.numero.focus();
      }
      else {
        alert('Erro carregar ingrediente: ' + retorno.mensagem);
      }
    }}).send(url);
  }
  else {
    //frameDoc.getElementById('cod_ingredientes_marcas_'+indice).value = '';
    //alert('Para carregar selecione um ingrediente válido.');
  }
}

function adicionar_ingrediente(indice)
{
  var iframe = document.getElementById('upload_nfe');
  var frameDoc = iframe.contentDocument || iframe.contentWindow.document;

  var cod_ingredientes = frameDoc.getElementById('cod_ingredientes_'+indice).value;

  //var el = frameDoc.getElementById(elementID);
  //el.parentNode.removeChild(el);

  ///alert(frameDoc);
  var valor_minimo = frameDoc.getElementById('entrada_estoque_minima_' + indice).value;
  var valor_maximo =frameDoc.getElementById('entrada_estoque_maxima_' + indice).value;
	var quantidade = frameDoc.getElementById('quantidade_ingredientes_adicionar_' + indice).value;
  quantidade = quantidade.replace(",",".");
	var quantidade_embalagem = frameDoc.getElementById('quantidade_ingredientes_adicionar_embalagem_' + indice).value;
  quantidade_embalagem = quantidade_embalagem.replace(",",".");
	/*var preco = $('preco_ingredientes_adicionar_' + indice).value;*/
  var preco_total = frameDoc.getElementById('preco_total_ingredientes_adicionar_' + indice).value;
  var unidade = frameDoc.getElementById('tipo_unidade_' + indice).value;
  var divisor_comum = frameDoc.getElementById('divisor_comum_' + indice).value;
  var unidade_trib = frameDoc.getElementById('unidade_compra_nota_' + indice).value;
  var cean_trib = frameDoc.getElementById('cean_trib_' + indice).value;
  var cod_fornecedores_item = frameDoc.getElementById('cod_fornecedores_item_' + indice).value;
  ...
  var liberacao = 0;
	...
	if(quantidade_embalagem != 0)
	{
       ...
       // alert(total_entrada*divisor_comum);
        ...
        if (liberacao>0)
        {
  				var url = 'acao=adicionar_ingrediente&cod_ingredientes=' + cod_ingredientes + '&quantidade=' + quantidade + '&quantidade_embalagem=' + quantidade_embalagem + '&unidade=' + unidade +'&divisor_comum='+divisor_comum+'&liberacao='+liberacao + '&preco_total=' + preco_total + '&cean_trib='+cean_trib+'&unidade_trib='+unidade_trib+'&cod_fornecedores_item='+cod_fornecedores_item; //'&preco=' + preco + 
                ...
                frameDoc.getElementById("confirmacao_"+indice).value = "2";
			...
			else
			{
				...
				//$('preco_ingredientes_adicionar_' + cod_ingredientes_marcas).focus();
			}
		}
		else
		{
			alert('Digite a quantidade antes de adicionar.');
			//$('quantidade_ingredientes_adicionar_' + cod_ingredientes_marcas).focus();
		}
	}
	else
	{
		alert('Digite a quantidade da embalagem antes de adicionar.');
		//$('quantidade_ingredientes_adicionar_embalagem_' + indice).focus();
	}
  
}

function confirmar_ingredientes(indice,checkbox)
{
  var iframe = document.getElementById('upload_nfe');
  var frameDoc = iframe.contentDocument || iframe.contentWindow.document;
  ...
  if(frameDoc.getElementById("confirmacao_"+indice).value == "1")
  {
    var tds = frameDoc.getElementById("linha_"+indice).getElementsByTagName("td");
    tds[0].style.backgroundColor = 'white';
    tds[1].style.backgroundColor = 'white';
    tds[2].style.backgroundColor = 'white';
    tds[3].style.backgroundColor = 'white';
    tds[4].style.backgroundColor = 'white';
    tds[5].style.backgroundColor = 'white';
    tds[6].style.backgroundColor = 'white';
    frameDoc.getElementById("confirmacao_"+indice).value = "0";

    var inputs = frameDoc.getElementById("linha_"+indice).getElementsByTagName("input");
    for( var i in inputs)
    {
      if(inputs[i].type=="text")
      inputs[i].readOnly = false;
    }
    frameDoc.getElementById("ingrediente_filtro_"+indice).disabled = false;
  }
  else if(frameDoc.getElementById("confirmacao_"+indice).value != "2")
  {
    if(quantidade_embalagem != 0)
    {
      if(quantidade > 0)
      {
        if(preco_total != 0)
        {
         ...
         // alert(total_entrada*divisor_comum);
          ...
          {
            var tds = frameDoc.getElementById("linha_"+indice).getElementsByTagName("td");
            tds[0].style.backgroundColor = 'darkseagreen';
            tds[1].style.backgroundColor = 'darkseagreen';
            tds[2].style.backgroundColor = 'darkseagreen';
            tds[3].style.backgroundColor = 'darkseagreen';
            tds[4].style.backgroundColor = 'darkseagreen';
            tds[5].style.backgroundColor = 'darkseagreen';
            tds[6].style.backgroundColor = 'darkseagreen';
            frameDoc.getElementById("confirmacao_"+indice).value = "1";
            ...
            frameDoc.getElementById("ingrediente_filtro_"+indice).disabled = true;
          }

        }
        else
        {
          alert('Digite o preço antes de confirmar.');
      ...
      {
        alert('Digite a quantidade antes de confirmar.');
        checkbox.checked = false;
        //$('quantidade_ingredientes_adicionar_' + cod_ingredientes_marcas).focus();
      }
    }
    ...
    {
      alert('Digite a quantidade da embalagem antes de confirmar.');
      checkbox.checked = false;
      //$('quantidade_ingredientes_adicionar_embalagem_' + indice).focus();
    }
  }
...
/*
function confirmar_ingredientes(indice)
{
  var iframe = document.getElementById('upload_nfe');
  var frameDoc = iframe.contentDocument || iframe.contentWindow.document;

  //tds[3].firstChild.value;


}
*/
function adicionar_todos()
{
  var iframe = document.getElementById('upload_nfe');
  var frameDoc = iframe.contentDocument || iframe.contentWindow.document;

  var quantidade_linhas =  frameDoc.getElementById('quantidade_linhas').value;

  for(var i = 0;i<quantidade_linhas;i++)
  {
    if(frameDoc.getElementById("confirmacao_"+i).value == "1")
    {
      adicionar_ingrediente(i);
    }
  }
...
{
  if(document.getElementById("cod_fornecedores"))
  {
    if(document.getElementById("cod_fornecedores").value!="")
    {
      var iframe = document.getElementById('upload_nfe');
      var frameDoc = iframe.contentDocument || iframe.contentWindow.document;

      frameDoc.getElementById('cod_forncedores_enviar').value = document.getElementById("cod_fornecedores").value;
      document.getElementById('upload_nfe').style.height = '400px';
  ...
  {
    alert("Selecione um Grupo e Subgrupo");
    return false;
  }
}
...
<form name="frmIncluir" method="post" onsubmit="{return (validar_nota())&& (validaRequeridos(this))}">
	...
	<tr>
        <td class="legenda tdbl tdbr tdbt">
            <label for="cod_pizzarias" class="requerido"><? echo ucfirst(TIPO_PRODUTO) ?></label>
        </td>
    </tr>
    <tr>
    ...
    <tr>
        <td class="tdbl tdbr sep" >
          <iframe id="upload_nfe" name="upload_nfe" src="ipi_estoque_entrada_nfe_ajax.php" style="width:100%;height:100px;border:1px solid #fff;"></iframe>
        </td>
    </tr>
    <!-- <tr>
        <td class="legenda tdbl tdbr sep">
