...
function atualizar_grafico_geral()
{
    ...
    var grafico_geral = new FusionCharts('../lib/swf/fusioncharts/msline.swf', 
                                         'grafico', 690, 330, 0, 0, 'ffffff', 0);

    grafico_geral.setDataURL('ipi_rel_ger_volume_vendas_ajax.php?param=<? echo date('dmYHis'); ?>,1,' + ano + ',' + cod_pizzarias + ',' + origem);
    grafico_geral.render('grafico_geral');
}

function atualizar_resumo_geral()
{
    var ano = document.getElementById('ano').value;
    var cod_pizzarias = document.getElementById('cod_pizzarias').value;
    var origem = document.getElementById('origem').value;
    var url = 'param=<? echo date("dmYHis"); ?>,2,' + ano + ',' + cod_pizzarias + ',' + origem;
    
    $('barra_loader').setStyle("display","block");
    $('resumo_geral').setStyle("display","none");
    new Request.HTML(
    {
        url: 'ipi_rel_ger_volume_vendas_ajax.php',
        update: $('resumo_geral'),
        onComplete: function(response) { $('barra_loader').setStyle("display","none"); $('resumo_geral').setStyle("display","block"); }
    }).send(url);
}
...
<table align="center" class="caixa" cellpadding="0" cellspacing="0">
    ...
    <tr>
        ...
        <td class="tdbr"><select name="cod_pizzarias" id="cod_pizzarias"
            ...
            {
            	echo '<option value="0">Todas as '.ucfirst(TIPO_EMPRESAS) .'</option>';	
            }
    ...
    <tr>
        ...
        <td class="tdbr sep"><select name="origem" id="origem"
            ...
            <option value="NET" <? if ($origem == 'NET') { echo 'selected'; } ?>>Net & IFOOD</option>
    ...
    <tr>
        <td align="right" class="tdbl tdbb tdbr" colspan="3"><input
            class="botaoAzul" type="button" value="Buscar" onclick="click_buscar()"></td>
    </tr>
...
<table align="center" style="margin: 0px auto;" cellpadding="0" cellspacing="0" width='1000'>
    <tr>
        <td width="700"><div id="grafico_geral"></div></td>
        <td width="300" align='right'><div id="barra_loader" style="display: none" align="center">
      Calculando...<br />
      <img src="../lib/img/principal/ajax_loader_barra.gif" />
    </div>
    <div id="resumo_geral"></div></td>
    </tr>
</table>
