...
/**
 * Cadastro de Tipos de Colaboradores
 *
 * @version 1.0
 * @package ipizza
 * 
...
cabecalho('Cadastro de Tipos de Colaboradores');

$acao = validaVarPost('acao');

$chave_primaria = 'cod_tipo_colaboradores';
$tabela = 'ipi_tipo_colaboradores';
$campo_ordenacao = 'cod_tipo_colaboradores';
$campo_filtro_padrao = 'tipo_colaboradores';
$quant_pagina = 50;
...
switch ($acao)
{
    ...
    case 'editar':
        ...
        $tipo_colaboradores = validaVarPost('tipo_colaboradores');
        
        $conexao = conectabd();
        
        if ($codigo <= 0)
        {
            $sql_edicao = sprintf("INSERT INTO $tabela (tipo_colaboradores) VALUES ('%s')", $tipo_colaboradores);
        ...
        {
            $sql_edicao = sprintf("UPDATE $tabela SET tipo_colaboradores = '%s' WHERE $chave_primaria = $codigo", $tipo_colaboradores);
...
window.addEvent('domready', function()
{
    ...
    tabs.addEvent('change', function(indice)
    {
        if(indice == 1)
        {
            ...
            document.frmIncluir.tipo_colaboradores.value = '';
        ...
        <table class="listaEdicao" cellpadding="0" cellspacing="0" width="<?echo LARGURA_PADRAO?>">
            <thead>
                <tr>
                    ...
                    <td align="center">Tipo de Colaborador</td>
                </tr>
            </thead>
            <tbody>
            ...
            while ($obj_buscar_registros = mysql_fetch_object($res_buscar_registros))
            {
                ...
                echo '<td align="center"><a href="#" onclick="editar(' . $obj_buscar_registros->$chave_primaria . ')">' . bd2texto($obj_buscar_registros->tipo_colaboradores) . '</a></td>';
...
<table align="center" class="caixa" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td class="legenda tdbl tdbt tdbr">
            <label class="requerido" for="tipo_colaborador">Tipo de Colaborador</label>
        </td>
    </tr>
    <tr>
        <td class="tdbl tdbr sep">
            <input class="requerido" type="text" name="tipo_colaboradores" id="tipo_colaboradores" maxlength="45" size="45" value="<?echo bd2texto($obj_editar->tipo_colaboradores)?>">
        </td>
    </tr>
