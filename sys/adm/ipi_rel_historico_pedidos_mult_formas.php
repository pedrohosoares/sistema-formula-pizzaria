...
cabecalho('Histórico de Pedidos com Múltiplas Formas de Pagamento');
...
<!--   
 ...
 -->
  ...
  <tr>
    <td class="legenda tdbl" align="right"><label for="entrega">Tipo de Entrega:</label></td>
    <td class="">&nbsp;</td>
    <td class="tdbr">
  ...
  <tr>
    <td class="legenda tdbl " align="right"><label for="tempo_envio">Tempo de Envio:</label></td>
    <td class="">&nbsp;</td>
    <td class="tdbr ">
  ...
  <tr>
    <td class="legenda tdbl " align="right"><label for="tempo_envio">Forma de Pagamento:</label></td>
    <td class="">&nbsp;</td>
    <td class="tdbr ">

    <select name="cod_formas_pg">
      <option value="">Todas</option>
        <?
        $sql_formas_pg = "SELECT fp.forma_pg,fp.cod_formas_pg FROM ipi_formas_pg fp ORDER BY forma_pg";
        ...
        while($obj_formas_pg = mysql_fetch_object($res_formas_pg)) 
        {
            ...
            if($cod_formas_pg == $obj_formas_pg->cod_formas_pg)
  ...
  <tr>
    <td class="legenda tdbl " align="right"><label for="tempo_envio">Atendente:</label></td>
    <td class="">&nbsp;</td>
    <td class="tdbr ">
    ...
    <select name="cod_usuarios">
      <option value="">Todas</option>
        ...
        $sql_formas_pg = "SELECT u.nome, u.cod_usuarios FROM ipi_caixa c INNER JOIN nuc_usuarios u ON (u.cod_usuarios = c.cod_usuarios_abertura) WHERE c.data_hora_abertura >= '".data2bd($data_inicial)." 00:00:00' AND c.data_hora_fechamento <= '".data2bd($data_final)." 23:59:59' GROUP BY u.cod_usuarios ORDER BY u.nome ";
        $res_formas_pg = mysql_query($sql_formas_pg);
        while($obj_formas_pg = mysql_fetch_object($res_formas_pg)) 
        {
            echo '<option value="'.$obj_formas_pg->cod_usuarios.'" ';
            if($cod_usuarios == $obj_formas_pg->cod_usuarios)
                echo 'selected="selected"';
            echo '>'.bd2texto($obj_formas_pg->nome).'</option>';
        }
        desconectabd($con);
        ?>
    </select>
...
if($acao!="")
{
  ...
  $SqlBuscaRegistros = "SELECT p.*, c.*, pi.nome AS pi_nome, pfp.valor as valor_parcela, fp.forma_pg as forma_pg_parcela, ca.cod_usuarios_abertura, u.nome usuario_abertura , p.situacao AS pedidos_situacao,(SELECT CASE WHEN unix_timestamp(p.data_hora_envio) >= (unix_timestamp(DATE_ADD(p.data_hora_pedido, INTERVAL 30 MINUTE))-600) and unix_timestamp(p.data_hora_envio) <= unix_timestamp(DATE_ADD(p.data_hora_pedido, INTERVAL 30 MINUTE)) THEN 'amarelo' WHEN unix_timestamp(p.data_hora_envio) >= unix_timestamp(DATE_ADD(p.data_hora_pedido, INTERVAL 30 MINUTE)) THEN 'vermelho' ELSE 'branco' END ) AS tempo_entrega  FROM $tabela p INNER JOIN ipi_clientes c ON (p.cod_clientes = c.cod_clientes) INNER JOIN ipi_pizzarias pi ON (p.cod_pizzarias = pi.cod_pizzarias) INNER JOIN ipi_pedidos_formas_pg pfp ON (p.cod_pedidos = pfp.cod_pedidos) LEFT JOIN ipi_formas_pg fp ON (pfp.cod_formas_pg = fp.cod_formas_pg) LEFT JOIN ipi_caixa_ipi_pedidos cp ON (cp.cod_pedidos = p.cod_pedidos) LEFT JOIN ipi_caixa ca ON (cp.cod_caixa = ca.cod_caixa) LEFT JOIN nuc_usuarios u ON (u.cod_usuarios = ca.cod_usuarios_abertura) WHERE p.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") ";
...
/*    if(strtotime($objBuscaRegistros->data_hora_envio)>=strtotime($objBuscaRegistros->tempo_entrega))
    ...
    $SqlSomaTotal = "SELECT sum(pfp.valor) as total_geral FROM $tabela p LEFT JOIN ipi_caixa_ipi_pedidos cp ON (cp.cod_pedidos = p.cod_pedidos) LEFT JOIN ipi_caixa ca ON (cp.cod_caixa = ca.cod_caixa) INNER JOIN ipi_pedidos_formas_pg pfp ON (p.cod_pedidos = pfp.cod_pedidos) WHERE p.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ")  ";
  ...
  if($cod_formas_pg > 0)
  {
    $SqlBuscaRegistros .= " AND pfp.cod_formas_pg = '$cod_formas_pg'";
    $SqlSomaTotal .= " AND pfp.cod_formas_pg = '$cod_formas_pg'";
  }
  ...
  if($cod_usuarios > 0)
  {
    $SqlBuscaRegistros .= " AND ca.cod_usuarios_abertura = '$cod_usuarios'";
    $SqlSomaTotal .= " AND ca.cod_usuarios_abertura = '$cod_usuarios'";
  }
  ...
  <table class="listaEdicao" cellpadding="0" cellspacing="0">
    <thead>
      ...
      <tr>
        ...
        <td align="center" width="120">Atendente</td>
        ...
        <td align="center" width="70">Valor Parcela</td>
        <td align="center" width="70">Total Pedido</td>
        <td align="center" width="70">Tipo Entrega</td>
      </tr>
    </thead>
    <tbody>
    ...
    if($acao!="")
    {
      ...
      while($objBuscaRegistros = mysql_fetch_object($resBuscaRegistros)) {
        ...
        echo '<td align="center" '.$cor.'>'.$objBuscaRegistros->usuario_abertura.'</td>';
        ...
        {
          echo '<td align="center" '.$cor.'>'.bd2texto($objBuscaRegistros->bairro).', '.bd2texto($objBuscaRegistros->endereco).' '.bd2texto($objBuscaRegistros->numero).' comp.:'.bd2texto($objBuscaRegistros->complemento).'</td>';
        }
        else
        {
          echo '<td align="center" '.$cor.'>Balcão</td>';
        }
        echo '<td align="center" '.$cor.'>'.$objBuscaRegistros->forma_pg_parcela.'</td>';
        ...
        echo '<td align="center" '.$cor.'>'.bd2moeda($objBuscaRegistros->valor_parcela).'</td>';
