...
for ($a = 0; $a < $num_formas_pg; $a++)
{
    ...
    echo '<td style="background-color: #e5e5e5;" align="center"><b>' . $obj_formas_pg->forma_pg . '</b></td>';
     ...
     $objBuscaPedidosSoma = executaBuscaSimples("SELECT SUM(pfp.valor) AS soma_tel FROM ipi_pedidos p LEFT JOIN ipi_pedidos_formas_pg pfp ON (p.cod_pedidos = pfp.cod_pedidos) WHERE p.data_hora_pedido BETWEEN '$data_inicial_sql' AND '$data_final_sql' AND pfp.cod_formas_pg = '".$obj_formas_pg->cod_formas_pg."' AND p.origem_pedido = 'TEL' AND p.cod_pizzarias = $cod_pizzarias AND p.situacao = 'BAIXADO'", $con);
    ...
    $objBuscaPedidosSoma = executaBuscaSimples("SELECT SUM(pfp.valor) AS soma_net FROM ipi_pedidos p LEFT JOIN ipi_pedidos_formas_pg pfp ON (p.cod_pedidos = pfp.cod_pedidos) WHERE p.data_hora_pedido BETWEEN '$data_inicial_sql' AND '$data_final_sql' AND pfp.cod_formas_pg = '".$obj_formas_pg->cod_formas_pg."' AND p.origem_pedido = 'NET' AND p.cod_pizzarias = $cod_pizzarias AND p.situacao = 'BAIXADO'", $con);
...
$sql_num_tickets = "SELECT count(cod_pedidos) as contagem, sum(valor_total) as total, (sum(valor_total)/count(cod_pedidos)) as ticket_medio FROM ipi_pedidos p WHERE p.origem_pedido = 'NET' $sql_buscar_pizzarias $sql_buscar_clientes $sql_data_hora_pedido $sql_situacao_pedido";
$res_num_tickets = mysql_query($sql_num_tickets);
$obj_num_tickets = mysql_fetch_object($res_num_tickets);
...
$sql_net = "SELECT count(cod_pedidos) as contagem, sum(valor) as total, (sum(valor)/count(cod_pedidos)) as ticket_medio FROM ipi_pedidos p WHERE p.origem_pedido = 'NET' $sql_buscar_pizzarias $sql_buscar_clientes $sql_data_hora_pedido $sql_situacao_pedido";*/
