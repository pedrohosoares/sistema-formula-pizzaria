...
cabecalho('Adicionais Mais Vendidos');
...
$con = conectabd();
$filtroSql="";
$SqlBuscaRegistros = "SELECT i.ingrediente, (SELECT COUNT(*) FROM ipi_pedidos_fracoes pf INNER JOIN ipi_pedidos_pizzas pp ON (pf.cod_pedidos_pizzas = pp.cod_pedidos_pizzas AND pf.cod_pedidos = pp.cod_pedidos) INNER JOIN ipi_pedidos pe ON (pp.cod_pedidos = pe.cod_pedidos) INNER JOIN ipi_pedidos_ingredientes pi ON (pf.cod_pedidos = pi.cod_pedidos AND pf.cod_pedidos_pizzas = pi.cod_pedidos_pizzas AND pf.cod_pedidos_fracoes = pi.cod_pedidos_fracoes) WHERE pe.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND  pi.cod_ingredientes=i.cod_ingredientes AND pi.ingrediente_padrao = 0 ";

if($origem != 'TODOS')
    $filtroSql .= " AND pe.origem_pedido = '$origem'";

if($origem == 'NET')
    $filtroSql .= " AND pe.origem_pedido IN ('NET','IFOOD')";

if($cod_pizzarias)
    $filtroSql .= " AND pe.cod_pizzarias = '$cod_pizzarias'";
    
if($cod_tamanhos)
    $filtroSql .= " AND pp.cod_tamanhos = '$cod_tamanhos'";
    
if(($data_inicial) && ($data_final)) 
{
    $data_inicial_sql = data2bd($data_inicial).' 00:00:00'; 
    $data_final_sql = data2bd($data_final).' 23:59:59';
    $filtroSql .= " AND pe.data_hora_pedido >= '$data_inicial_sql' AND pe.data_hora_pedido <= '$data_final_sql'";
}

if($promocional == 'Sim')
  $filtroSql .= " AND pp.fidelidade = 1 AND pp.promocional = 1";
else if($promocional == 'Não')
  $filtroSql .= " AND pp.fidelidade = 0 AND pp.promocional = 0";

if($situacao != 'TODOS')
  $filtroSql .= " AND pe.situacao = '$situacao'";


$SqlBuscaRegistros .= " $filtroSql ) quantidade, (SELECT SUM(pi.preco) FROM ipi_pedidos_fracoes pf INNER JOIN ipi_pedidos_pizzas pp ON (pf.cod_pedidos_pizzas = pp.cod_pedidos_pizzas AND pf.cod_pedidos = pp.cod_pedidos) INNER JOIN ipi_pedidos pe ON (pp.cod_pedidos = pe.cod_pedidos) INNER JOIN ipi_pedidos_ingredientes pi ON (pf.cod_pedidos = pi.cod_pedidos AND pf.cod_pedidos_pizzas = pi.cod_pedidos_pizzas AND pf.cod_pedidos_fracoes = pi.cod_pedidos_fracoes) WHERE pe.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND  pi.cod_ingredientes=i.cod_ingredientes AND pi.ingrediente_padrao = 0 $filtroSql ) valor FROM ipi_ingredientes i ORDER BY ingrediente";
...
while($objBuscaRegistros = mysql_fetch_object($resBuscaRegistros)) 
{
    ...
    echo '<td align="center">'.($objBuscaRegistros->valor!="" ? "R$ ".bd2moeda($objBuscaRegistros->valor) : "-").'</td>';
    echo '</tr>';
    $total_geral += $objBuscaRegistros->quantidade;
    $valorTotal += $objBuscaRegistros->valor;
}
desconectabd($con);
?>
  ...
  <tr>
    ...
    <td align="center"><b><? echo ($valorTotal>0 ?"R$ ". bd2moeda($valorTotal) : $valorTotal) ?></b></td>
  </tr>
