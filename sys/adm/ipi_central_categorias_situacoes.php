...
cabecalho('Cadastro de Situações para tickets');

$acao = validaVarPost('acao');

$chave_primaria = 'cod_situacoes';
$tabela = 'ipi_comunicacao_categorias_situacoes';
...
switch ($acao)
{
    ...
    case 'editar':
        ...
        $nome_situacao = validaVarPost('nome_situacao');
        $situacao = validaVarPost('situacao');
        $conexao = conectabd();
        
        if ($codigo <= 0)
        {
            $sql_edicao = sprintf("INSERT INTO $tabela (cod_categorias,nome_situacao,situacao) VALUES (%d,'%s','%s')", $cod_categoria,$nome_situacao,$situacao);
        ...
        {
            $sql_edicao = sprintf("UPDATE $tabela SET cod_categorias = %d, situacao = '%s',nome_situacao = '%s' WHERE $chave_primaria = $codigo", $cod_categoria, $situacao,$nome_situacao);
            $res_edicao = mysql_query($sql_edicao);
        }
...
<div id="tabs">
        ...
        <table class="listaEdicao" cellpadding="0" cellspacing="0">
          <thead>
            <tr>
              ...
              <td align="center">Situação da situação</td>
            </tr>
          </thead>
          <tbody>
          ...
          $SqlBuscaRegistros = "SELECT * FROM $tabela s INNER JOIN ipi_comunicacao_categorias c ON (s.cod_categorias = c.cod_categorias) WHERE c.cod_categorias='".validaVarPost('cod_categorias')."' ORDER BY $chave_primaria";
          ...
          while ($objBuscaRegistros = mysql_fetch_object($resBuscaRegistros)) {
            ...
            echo '<td align="center"><a href="javascript:;" onclick="editar('.$objBuscaRegistros->$chave_primaria.')">'.bd2texto($objBuscaRegistros->nome_situacao).'</a></td>';
  ...
  <div class="painelTab">
    ...
    <tr><td class="legenda tdbl tdbr"><label class="requerido" for="nome_resposta">Nome da Situação</label></td></tr>
    <tr><td class="tdbl tdbr sep"><input class="requerido" type="text" name="nome_situacao" id="nome_situacao" maxlength="100" size="50" value="<? echo texto2bd($objBusca->nome_situacao) ?>"></td></tr>
