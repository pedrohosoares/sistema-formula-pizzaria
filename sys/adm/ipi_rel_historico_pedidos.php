      ...
      <tbody>
        ...
        if($acao!="")
        {
          ...
          while($objBuscaRegistros = mysql_fetch_object($resBuscaRegistros)) {
            if($objBuscaRegistros->pedidos_situacao == 'BAIXADO'){
              $sqlSituacao = "Múltiplas";
            }
            $cor = '';
            ...
            if(empty($objBuscaRegistros->cancelamento_json) and !empty($arquivoJson) and isset($arquivoJson['caminho_danfe']) and isset($arquivoJson['qrcode_url'])){
              ...
              <td align="center" <?php echo $cor; ?>>
                <a onclick="return confirm('Você tem certeza que quer cancelar o pedido?')" href="cancelar_nota.php?ref=<?php echo $objBuscaRegistros->cod_pedidos; ?>">  
                  CANCELAR NOTA / PEDIDO
                </a>
              </td>
              <?php
              ...
              if(!empty($objBuscaRegistros->cancelamento_json)){
                $cancelamento = json_decode($objBuscaRegistros->cancelamento_json,true);
                if(isset($cancelamento['status']) and $cancelamento['status'] == 'cancelado'){
                  echo '<td align="center" '.$cor.'>NOTA CANCELADA</td>';
                  echo '<td align="center" '.$cor.'>NOTA CANCELADA</td>';
                }else{
                  echo '<td align="center" '.$cor.'><a target="_blank" href="https://api.focusnfe.com.br'.$arquivoJson['caminho_danfe'].'" style="display:block;border: 1px solid #333;margin: 3px;background: yellow;padding: 2px;">CUPOM FISCAL</a>';
                ...
                if($objBuscaRegistros->pedidos_situacao != 'CANCELADO'){
                  ...
                  echo '<td align="center" '.$cor.'>Pedido já cancelado</td>';
                }
              }
            }
            ...
            if($objBuscaRegistros->pedidos_situacao == 'BAIXADO'){
              echo '<td align="center" '.$cor.'>'.$sqlSituacao.'</td>';
            }else{
            ...
            if($objBuscaRegistros->pedidos_situacao == 'CANCELADO'){
              $mensagem = "";
              if(!empty($objBuscaRegistros->cancelamento_json)){
                $mensagem = json_decode($objBuscaRegistros->cancelamento_json,true);
                if($mensagem['justificativa']){
                  $mensagem = $mensagem['justificativa'];
                }else{
                  $mensagem = "";
                }
              }
