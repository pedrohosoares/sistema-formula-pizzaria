...
cabecalho('Relatório de Utilização dos Cupons pela Agência');
...
function trocar(cod_cupons,cliente,cod_pizzarias) 
{
  var idTrocar = 'result_'+cod_cupons;
  var controle = 'controle_'+cod_cupons;
  if (document.getElementById(idTrocar).style.display=="none")
  {
    if(document.getElementById(controle).value==0)
    {
      carregaDetalhes(cod_cupons,cliente,cod_pizzarias);
...
function carregaDetalhes(cod_cupons,cliente,cod_pizzarias) 
{
  var url = 'acao=carregar_detalhes';
  
  url += '&c=' + cod_cupons+'&s=TODOS&cli='+cliente+'&p='+cod_pizzarias;
  
  $('tbody_'+cod_cupons).set('text', 'Carregando...');
  
  new Request.HTML({
  url: 'ipi_rel_cupom_unidade_ajax.php',
  update: $('tbody_'+cod_cupons)
   }).send(url);
}
...
<div id="tabs">
  ...
  <form name="frmFiltro" method="post">
    ...
    <tr>
      ...
      $txtNumCupomFiltro = $_SESSION['usuario']['email'];
      /*
      <input class="requerido" type="text" name="txtNumCupomFiltro" id="txtNumCupomFiltro" size="60" value="<? echo $txtNumCupomFiltro ?>" >
      */
      ?>
      </td>
    </tr>
    ...
    <tr>
      <td class="legenda tdbl" align="right"><label for="cliente">Mídia:</label></td>
      <td class="">&nbsp;</td>
      <td class="tdbr">
      <?php
...
?>
    ...
    <tr><td align="right" class="tdbl tdbb tdbr" colspan="3"></td></tr>
    ...
    <input type="hidden" name="acao" value="buscar">
      ...
      <?
        ...
        $SqlBuscaRegistros = "SELECT cp.*,usu.nome, (SELECT count(pc.cod_pedidos) FROM ipi_pedidos_ipi_cupons pc INNER JOIN ipi_pedidos p ON (pc.cod_pedidos=p.cod_pedidos) INNER JOIN ipi_cupons ic ON (pc.cod_cupons=ic.cod_cupons) WHERE p.cod_pizzarias IN (" . $cod_pizzarias . " ) AND p.cod_pizzarias IN(".implode(',',$_SESSION['usuario']['cod_pizzarias']).") AND ic.cupom=cp.cupom $filtro_clientes) num_utilizacoes FROM $tabela cp inner join nuc_usuarios usu on usu.cod_usuarios = cp.usuario_criacao INNER JOIN ipi_pizzarias_cupons pic ON (pic.cod_cupons = cp.cod_cupons) WHERE pic.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") ";
        ...
        $SqlBuscaRegistros .= " ORDER BY cod_cupons DESC LIMIT 1";
        $resBuscaRegistros = mysql_query($SqlBuscaRegistros);
        $linhasBuscaRegistros = mysql_num_rows($resBuscaRegistros);

        if ($linhasBuscaRegistros > 0)
        {
          /*
          ...
          <table class="listaEdicao" cellpadding="0" cellspacing="0">
            <thead>
              <tr>
                ...
                <td align="center" width="100">Usuário de Criação</td>
              </tr>
            </thead>
            <tbody>
            ...
            while ($objBuscaRegistros = mysql_fetch_object($resBuscaRegistros)) {
              if($objBuscaRegistros->num_utilizacoes > 0) {
                ...
                echo '<td align="center" align="left" style="padding-left: 10px;"><strong>'.bd2texto($objBuscaRegistros->cupom).'</strong><br/>('.$objBuscaRegistros->obs_cupom.') </td>';
                ...
                /*$SqlBuscaPedidos = "SELECT pc.cod_pedidos,p.data_hora_pedido,c.nome,c.cod_clientes FROM ipi_pedidos_ipi_cupons pc INNER JOIN ipi_pedidos p ON (pc.cod_pedidos = p.cod_pedidos) INNER JOIN ipi_clientes c ON (p.cod_clientes = c.cod_clientes) WHERE p.cod_pizzarias IN (" . implode(',', $_SESSION['usuario']['cod_pizzarias']) . ") AND p.cod_pizzarias IN (".$cod_pizzarias.") AND cod_cupons = ".$objBuscaRegistros->cod_cupons." $filtro_clientes ORDER BY pc.cod_pedidos";
                $resBuscaPedidos = mysql_query($SqlBuscaPedidos);*/
                
                //echo $SqlBuscaPedidos;
                
                echo '<tr style="display: none;" id="result_'.$objBuscaRegistros->cod_cupons.'">';
                echo '<td colspan="5" align="left" style="padding: 20px;">';
                 echo '<input type="hidden" id="controle_'.$objBuscaRegistros->cod_cupons.'" value="0"/>';
                ...
                echo '</thead>';
                echo '<tbody id="tbody_'.$objBuscaRegistros->cod_cupons.'">';
        ...
        <?php
          ...
          echo "<br /><br /><center><strong>Nenhum Registro Encontrado!</strong></center>";
        ?>
