...
<body style='background:none !important'>
	...
	switch($acao) {
		...
		$cod_chapeiros = validaVarPost("cod_chapeiro");
		/*
		echo "<br/><br/><pre>";
		print_r($_POST);
		echo "</pre>";
...
/*			var chapeiro = document.getElementsByName('cod_chapeiro[]');

			for (var i = 0; i < chapeiro.length; i++) 
			{
				if(((chapeiro[i].type == 'radio')) && (checkBox[i].disabled != true) && ((chapeiro[i].checked == true))) { 
					cRadio++; 
				}
			}*/
			
			if(eRadio > 0/* && cRadio > 0*/)
			{
...
/*
				if(cRadio==0)
				{
					alert('Por favor, selecione o chapeiro para fazer a expedição.');
				}*/
	...
	function editar_entregadores() {
		...
		new Request.HTML(
		{
			url: 'ipi_despacho_todos_pedidos_ajax.php',
	...
	function despachar() {
		if(verificaCheckbox(document.frmBaixa)) {
			...
			foreach($codpizzarias as $i=>$v){
				if($v == '21' or $v == '13' or $v =='16' or $v == '12' or $v == '24' or $v == '20' or $v== '22' or $v=='23' or $v=='14' or $v=='9' or $v =='7' or $v =='1'){
					...
					var url = "http://sistema.formulapizzaria.com.br/formula/production/current/site/sys/adm/ipi_exibe_pedidos_despachados.php?"+id; 
...
<div id="preloader" style="display: none;">
	<div id="simbolo"></div>
	<p>Aguarde, emitindo NFC-e, não feche a janela...</p>
</div>
...
<form name="frmBaixa"  id="frmBaixa" method="post">
	...
	<div align="center" id='botao_despachar'><input style='height:40px;width:120px' class="botaoAzul despacharBtn" type="button" value="Despachar Pedidos" onclick="despachar()"> <input type="hidden" name="acao" value=""></div>
