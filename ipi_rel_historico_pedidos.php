...
switch($acao) {
  ...
  $sqlCozinha = "UPDATE $tabela SET tipo_reimpressao = 'cozinha' WHERE $chave_primaria IN ($indicesSql)";
  if (mysql_query($sqlCozinha))
    mensagemOk('O(s) pedido(s) da COZINHA foram definidos para a REIMPRESSÃO!');
  else
  ...
  $codigo = validaVarPost($chave_primaria);
  $indicesSql = implode(',', $codigo);
  $sqlMotoqueiro = "UPDATE $tabela SET tipo_reimpressao = 'motoqueiro' WHERE $chave_primaria IN ($indicesSql)";
  if (mysql_query($sqlMotoqueiro))
    mensagemOk('O(s) pedido(s) do MOTOQUEIRO foram definidos como REIMPRESSÃO com sucesso!');
  else
    mensagemErro('Erro ao REIMPRIMIR o pedido', 'Por favor, comunique a equipe de suporte informando todos os pedidos selecionados para definição.');
  ...
  foreach($session as $i=>$v){
    if($v == 24 or $v == 23 or $v==14 or $v == 22 or $v == 20 or $v == 9 or $v == 16 or $v == 7 or $v == 1 or $v == 21 or $v == 12 or $v == 13 or $v == 3 or $v == 17){
      $result =  true;
    }
  }
  $con = conectabd();
...
/*    if(strtotime($objBuscaRegistros->data_hora_envio)>=strtotime($objBuscaRegistros->tempo_entrega))
  ...
  <form name="frmBaixa" method="post">
    <table class="cabecalhoEdicao" cellpadding="0" cellspacing="0">
      <tr>
        <td align="left"><input class="botaoAzul" style="font-weight: bold; color: red;" type="button" value="Imprimir Cupom Motoqueiro" onclick="motoqueiro()"></td>
        <td align="left"><input class="botaoAzul" style="font-weight: bold; color: red;" type="button" value="Imprimir Cozinha" onclick="cozinha()"></td>
        <?php 
        $result = false;
        foreach($session as $i=>$v){
          if($v == 24 or $v == 23 or $v==14 or $v == 22 or $v == 20 or $v == 9 or $v == 16 or $v == 7 or $v == 1 or $v == 21 or $v == 12 or $v == 13 or $v == 3 or $v == 17){
